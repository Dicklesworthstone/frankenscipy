...
//!   `perf_sparse dia-to-csr-current-profile <n> <repeats>`
//!   `perf_sparse dia-to-csr-vs-scipy <n> <rounds> [oracle]`
...
mod expm_bench {
    ...
    use fsci_sparse::{
        BsrMatrix, CscMatrix, CsrMatrix, DiaMatrix, Shape2D, add_csc, sparse_transpose,
    ...
    const DIA_TO_CSR_N: usize = 65_536;
    const DIA_TO_CSR_DIAGONALS: usize = 33;
    const DIA_TO_CSR_NNZ: usize = DIA_TO_CSR_DIAGONALS * DIA_TO_CSR_N - 4_352;
    const DIA_TO_CSR_REGISTERED_ROUNDS: usize = 24;
    ...
    fn dia_to_csr_fixture() -> DiaMatrix {
        let mut offsets = Vec::with_capacity(DIA_TO_CSR_DIAGONALS);
        offsets.push(0);
        for step in 1..=16isize {
            offsets.push(-16 * step);
            offsets.push(16 * step);
        }
        let data = offsets
            .iter()
            .enumerate()
            .map(|(diagonal, offset)| {
                let length = DIA_TO_CSR_N - offset.unsigned_abs();
                (0..length)
                    .map(|position| (1 + ((13 * diagonal + 17 * position) % 251)) as f64 / 256.0)
                    .collect::<Vec<_>>()
            })
            .collect::<Vec<_>>();
        DiaMatrix::from_diagonals(Shape2D::new(DIA_TO_CSR_N, DIA_TO_CSR_N), offsets, data)
            .expect("canonical DIA-to-CSR fixture")
    }

    fn dia_input_sha256(matrix: &DiaMatrix) -> Result<String, String> {
        let mut digest = Sha256::new();
        for (label, value) in [
            ("row count", matrix.shape().rows),
            ("column count", matrix.shape().cols),
            ("diagonal count", matrix.num_diags()),
        ] {
            let value = u64::try_from(value)
                .map_err(|error| format!("DIA {label} does not fit u64: {error}"))?;
            digest.update(value.to_le_bytes());
        }
        for (&offset, diagonal) in matrix.offsets().iter().zip(matrix.data()) {
            let offset = i64::try_from(offset)
                .map_err(|error| format!("DIA offset does not fit i64: {error}"))?;
            digest.update(offset.to_le_bytes());
            let length = u64::try_from(diagonal.len())
                .map_err(|error| format!("DIA length does not fit u64: {error}"))?;
    ...
    impl ScipyExpm {
        ...
        fn start_dia_to_csr(script: &Path) -> Result<(Self, String), String> {
            Self::start_mode(script, "--live-dia-to-csr")
        }
        ...
        fn write_isize_vector(&mut self, label: &str, values: &[isize]) -> Result<(), String> {
            let mut line = String::with_capacity(label.len() + 2 + values.len() * 8);
            write!(line, "{label} ").map_err(|error| format!("format {label}: {error}"))?;
            for (index, value) in values.iter().enumerate() {
                if index != 0 {
        ...
        fn initialize_dia(&mut self, matrix: &DiaMatrix) -> Result<String, String> {
            writeln!(
                self.stdin,
                "INIT_DIA {} {} {} {}",
                matrix.shape().rows,
            ...
            .map_err(|error| format!("write INIT_DIA: {error}"))?;
            self.write_isize_vector("DIA_OFFSETS", matrix.offsets())?;
            let lengths = matrix.data().iter().map(Vec::len).collect::<Vec<_>>();
            self.write_usize_vector("DIA_LENGTHS", &lengths)?;
            let flattened = matrix
                .data()
                .iter()
                .flat_map(|diagonal| diagonal.iter().copied())
                .collect::<Vec<_>>();
            self.write_f64_vector("DIA_DATA", &flattened)?;
            self.stdin
                .flush()
                .map_err(|error| format!("flush INIT_DIA: {error}"))?;
            self.read_line()
        }
        ...
        fn dia_to_csr_parity(&mut self) -> Result<(String, String), String> {
            self.transpose_parity()
        }
    ...
    fn time_dia_to_csr(matrix: &DiaMatrix, repetitions: usize) -> f64 {
        let mut checksum = 0usize;
        let started = Instant::now();
        for _ in 0..repetitions {
            let result = matrix
                .to_csr()
                .expect("FrankenSciPy canonical DIA-to-CSR conversion");
            checksum ^= result
                .nnz()
                .wrapping_add(result.shape().rows)
                .wrapping_add(result.shape().cols);
    ...
    pub fn run_dia_to_csr_current_profile(n: usize, repetitions: usize) -> Result<(), String> {
        if n != DIA_TO_CSR_N || repetitions < 1 {
            return Err(format!(
                "registered DIA-to-CSR profile requires n={DIA_TO_CSR_N} and repetitions>=1"
            ));
        }
        let matrix = dia_to_csr_fixture();
        let input_sha256 = dia_input_sha256(&matrix)?;
        let result = matrix
            .to_csr()
            .map_err(|error| format!("FrankenSciPy DIA-to-CSR warmup: {error}"))?;
        if result.nnz() != DIA_TO_CSR_NNZ
            || !result.canonical_meta().sorted_indices
            || !result.canonical_meta().deduplicated
        {
            return Err("current DIA-to-CSR profile result is not canonical".to_string());
        }
        let elapsed = time_dia_to_csr(&matrix, repetitions);
        println!(
            "DIA_TO_CSR_FSCI_PROFILE n={n} diagonals={DIA_TO_CSR_DIAGONALS} \
             nnz={DIA_TO_CSR_NNZ} repetitions={repetitions} elapsed_seconds={elapsed:.9} \
    ...
    pub fn run_dia_to_csr_vs_scipy(
        n: usize,
        rounds: usize,
        explicit_oracle: Option<&String>,
    ) -> Result<(), String> {
        if n != DIA_TO_CSR_N || rounds != DIA_TO_CSR_REGISTERED_ROUNDS {
            return Err(format!(
                "one-shot profile requires n={DIA_TO_CSR_N} \
                 rounds={DIA_TO_CSR_REGISTERED_ROUNDS}"
            ));
        }
        ...
        let matrix = dia_to_csr_fixture();
        if matrix.shape() != Shape2D::new(DIA_TO_CSR_N, DIA_TO_CSR_N)
            || matrix.num_diags() != DIA_TO_CSR_DIAGONALS
            || matrix.nnz() != DIA_TO_CSR_NNZ
            || matrix
                .data()
                .iter()
                .flatten()
        ...
        {
            return Err("registered DIA fixture has the wrong contract".to_string());
        }
        let input_sha256 = dia_input_sha256(&matrix)?;
        let current = matrix
            .to_csr()
            .map_err(|error| format!("FrankenSciPy DIA-to-CSR parity: {error}"))?;
        if current.shape() != matrix.shape()
            || current.nnz() != DIA_TO_CSR_NNZ
            || current.indptr().len() != DIA_TO_CSR_N + 1
            || current.indptr()[0] != 0
            || current.indptr()[DIA_TO_CSR_N] != DIA_TO_CSR_NNZ
            || current
                .data()
                .iter()
                .any(|value| !value.is_finite() || *value <= 0.0)
        ...
        {
            return Err("FrankenSciPy DIA-to-CSR output contract failed".to_string());
        }
        let current_middle_pointer = current.indptr()[DIA_TO_CSR_N / 2];
        ...
        println!(
            "fixture=canonical-dia-to-csr n={DIA_TO_CSR_N} \
             diagonals={DIA_TO_CSR_DIAGONALS} nnz={DIA_TO_CSR_NNZ} \
             offsets=[0,-16,16,...,-256,256] \
             value=(1+((13*k+17*i)%251))/256 rounds={rounds} \
             construction_outside_timing=true serialization_outside_timing=true \
        ...
        let (mut scipy, identity) = ScipyExpm::start_dia_to_csr(&script)?;
        println!("scipy_arm: {identity}");
        if !identity.starts_with("READY scipy=1.17.1 ")
            || !identity.contains("method=dia_to_csr")
            || !identity.contains("solver_mod=scipy.sparse._dia")
            || !identity.contains("actual_observed_worker_threads=1")
            || !identity.contains("fsci_loaded=False")
            || !identity.contains("genuine=True")
        {
            return Err("live SciPy DIA-to-CSR failed genuine-incumbent identity gate".to_string());
        }
        let scipy_engine_sha256 = field_value(&identity, "scipy_engine_sha256=")
            .ok_or_else(|| "live SciPy omitted its DIA engine SHA-256".to_string())?;
        if !is_sha256(scipy_engine_sha256) {
            return Err("live SciPy reported an invalid DIA engine SHA-256".to_string());
        }
        println!("scipy_engine_sha256={scipy_engine_sha256}");

        let case = scipy.initialize_dia(&matrix)?;
        let expected_case = format!(
            "CASE method=dia_to_csr rows={DIA_TO_CSR_N} cols={DIA_TO_CSR_N} \
             diagonals={DIA_TO_CSR_DIAGONALS} compact_nnz={DIA_TO_CSR_NNZ} \
             matrix_nnz={DIA_TO_CSR_NNZ} finite=True result_format=csr \
             result_nnz={DIA_TO_CSR_NNZ} sorted=True canonical=True"
        );
        if case != expected_case {
            return Err(format!("live SciPy constructed the wrong DIA case: {case}"));
        }
        println!("scipy_case: {case}");
        let scipy_input_sha256 = scipy.input_sha256()?;
        ...
        let (live_result, live_output_sha256) = scipy.dia_to_csr_parity()?;
        let expected_result = format!(
            "RESULT rows={DIA_TO_CSR_N} cols={DIA_TO_CSR_N} nnz={DIA_TO_CSR_NNZ} \
             format=csr sorted=True canonical=True finite=True first_pointer=0 \
             middle_pointer={current_middle_pointer} last_pointer={DIA_TO_CSR_NNZ}"
        );
        if live_result != expected_result || live_output_sha256 != current_output_sha256 {
            return Err(format!(
                "DIA-to-CSR output contract mismatch: result={live_result} \
        ...
        println!(
            "agreement: current_output_sha256={current_output_sha256} \
             ...
             shape={DIA_TO_CSR_N}x{DIA_TO_CSR_N} nnz={DIA_TO_CSR_NNZ} exact=true"
        );

        let quiescence_pre = sample_host_wide_quiescence("pre")?;
        let mut current_repetitions = 1usize;
        let current_calibration_seconds = loop {
            let elapsed = time_dia_to_csr(&matrix, current_repetitions);
            if elapsed >= BSR_TO_CSR_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            current_repetitions = current_repetitions
                .checked_mul(2)
                .ok_or_else(|| "current DIA-to-CSR calibration overflowed".to_string())?;
        };
        let mut live_repetitions = 1usize;
        let live_calibration_seconds = loop {
            let elapsed = scipy.solve(live_repetitions, DIA_TO_CSR_NNZ)?;
            if elapsed >= BSR_TO_CSR_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            live_repetitions = live_repetitions
                .checked_mul(2)
                .ok_or_else(|| "live DIA-to-CSR calibration overflowed".to_string())?;
        };
        println!(
            "calibration: current_repetitions={current_repetitions} \
             live_repetitions={live_repetitions} min_sample_ms={} \
        ...
        for _ in 0..2 {
            let _ = time_dia_to_csr(&matrix, current_repetitions);
            let _ = scipy.solve(live_repetitions, DIA_TO_CSR_NNZ)?;
        }
        ...
        for round in 0..rounds {
            let (current_batch, live_batch) = if round.is_multiple_of(2) {
                (
                    time_dia_to_csr(&matrix, current_repetitions),
                    scipy.solve(live_repetitions, DIA_TO_CSR_NNZ)?,
                )
            } else {
                let live = scipy.solve(live_repetitions, DIA_TO_CSR_NNZ)?;
                let current = time_dia_to_csr(&matrix, current_repetitions);
                (current, live)
            };
            let current_null =
                four_call_geometric_null(|| Ok(time_dia_to_csr(&matrix, current_repetitions)))?;
            let live_null =
                four_call_geometric_null(|| scipy.solve(live_repetitions, DIA_TO_CSR_NNZ))?;
        ...
        let profile_admitted =
            ratio_high < 0.50 && null_medians_ok && clears_null && quiescence_all_clear;
        println!(
            "timing: current_p50_ms={:.6} current_p95_ms={:.6} current_p99_ms={:.6} \
             live_scipy_p50_ms={:.6} live_scipy_p95_ms={:.6} live_scipy_p99_ms={:.6} \
             incumbent_ratio_scipy_over_frankenscipy={ratio_median:.6} \
        ...
        println!(
            "registered_loss_gate: profile_admitted={profile_admitted} \
             ratio_ci_high={ratio_high:.6} required_ratio_ci_high_lt=0.500000 \
...
fn main() {
    ...
    if mode == "dia-to-csr-current-profile" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let n = args
                .get(2)
            ...
            let repetitions = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(1);
            if let Err(error) = expm_bench::run_dia_to_csr_current_profile(n, repetitions) {
        ...
        {
            eprintln!("DIA-to-CSR profiling requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
    if mode == "dia-to-csr-vs-scipy" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let n = args
                .get(2)
            ...
            let rounds = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(24);
            if let Err(error) = expm_bench::run_dia_to_csr_vs_scipy(n, rounds, args.get(4)) {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
            return;
        ...
        #[cfg(not(feature = "sparse-incumbent-bench"))]
        {
            eprintln!("DIA-to-CSR comparison requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
