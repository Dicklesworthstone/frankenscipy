...
//!   `perf_sparse coo-add-current-profile <n> <repeats>`
//!   `perf_sparse coo-add-vs-scipy <n> <rounds> [oracle]`
...
mod expm_bench {
    ...
    use fsci_sparse::{
        BsrMatrix, CooMatrix, CscMatrix, CsrMatrix, FormatConvertible, Shape2D, add_coo, add_csc,
    ...
    const COO_ADD_N: usize = 16_384;
    const COO_ADD_ENTRIES_PER_ROW: usize = 48;
    const COO_ADD_OPERAND_NNZ: usize = COO_ADD_N * COO_ADD_ENTRIES_PER_ROW;
    const COO_ADD_RESULT_NNZ: usize = COO_ADD_N * 72;
    const COO_ADD_REGISTERED_ROUNDS: usize = 24;
    ...
    const COO_ADD_MIN_SAMPLE_SECONDS: f64 = 0.020;
    ...
    fn coo_add_operand(side: usize) -> CooMatrix {
        let mut rows = Vec::with_capacity(COO_ADD_OPERAND_NNZ);
        let mut cols = Vec::with_capacity(COO_ADD_OPERAND_NNZ);
        let mut data = Vec::with_capacity(COO_ADD_OPERAND_NNZ);
        for row in 0..COO_ADD_N {
            let mut entries = (0..COO_ADD_ENTRIES_PER_ROW)
                .map(|slot| {
                    let column = (257 * slot + 31 * row + 6_168 * side) % COO_ADD_N;
                    let value = (1 + ((13 * row + 17 * slot + 23 * side) % 251)) as f64 / 256.0;
                    (column, value)
                })
                .collect::<Vec<_>>();
            entries.sort_unstable_by_key(|entry| entry.0);
        ...
        CooMatrix::from_triplets(Shape2D::new(COO_ADD_N, COO_ADD_N), data, rows, cols, false)
            .expect("canonical COO-add operand")
    }

    fn coo_add_fixture() -> (CooMatrix, CooMatrix) {
        (coo_add_operand(0), coo_add_operand(1))
    }

    fn coo_is_strictly_lexicographic(matrix: &CooMatrix) -> bool {
        (1..matrix.nnz()).all(|index| {
            let previous = (
                matrix.row_indices()[index - 1],
                matrix.col_indices()[index - 1],
            );
            let current = (matrix.row_indices()[index], matrix.col_indices()[index]);
            previous < current
        })
    }
    ...
    fn sample_coo_add_quiescence(phase: &str) -> Result<bool, String> {
        ...
        for (cpu, start) in &before {
            ...
            match *cpu {
                PINNED_CPU => pinned_busy = Some(busy_fraction),
                SMT_SIBLING => sibling_busy = Some(busy_fraction),
                _ => maximum_other_busy = maximum_other_busy.max(busy_fraction),
            }
        }
        let pinned_busy = pinned_busy.ok_or_else(|| format!("cpu{PINNED_CPU} is absent"))?;
        let sibling_busy = sibling_busy.ok_or_else(|| format!("cpu{SMT_SIBLING} is absent"))?;
        let host_mean_busy = if total_ticks == 0 {
            1.0
        } else {
            total_ticks.saturating_sub(idle_ticks) as f64 / total_ticks as f64
        };
        let host_iowait = if total_ticks == 0 {
            1.0
        } else {
        ...
        let clear = pinned_busy <= MAX_BUSY
            ...
            && host_mean_busy <= MAX_BUSY
            && host_iowait <= MAX_IOWAIT;
        println!(
            "registered_quiescence_{phase}={} pinned_cpu={PINNED_CPU} \
             pinned_busy_fraction={pinned_busy:.3} smt_sibling={SMT_SIBLING} \
             sibling_busy_fraction={sibling_busy:.3} host_mean_busy_fraction={host_mean_busy:.3} \
             host_iowait_fraction={host_iowait:.3} maximum_other_cpu_busy_fraction={maximum_other_busy:.3} \
             busy_limit={MAX_BUSY:.3} iowait_limit={MAX_IOWAIT:.3} \
             other_cpu_max_is_provenance_only=true evidence_class=PROVISIONAL_NON_EXCLUSIVE",
            if clear { "clear" } else { "NOT_CERTIFIED" }
        );
        Ok(clear)
    }
    ...
    fn coo_pair_input_sha256(lhs: &CooMatrix, rhs: &CooMatrix) -> Result<String, String> {
        let mut digest = Sha256::new();
        for (label, matrix) in [("left", lhs), ("right", rhs)] {
            for (field, value) in [
                ("row count", matrix.shape().rows),
                ...
                let value = u64::try_from(value)
                    .map_err(|error| format!("{label} COO {field} does not fit u64: {error}"))?;
                digest.update(value.to_le_bytes());
            }
            for &value in matrix.data() {
                digest.update(value.to_le_bytes());
            ...
            for (field, coordinates) in [
                ("row coordinate", matrix.row_indices()),
                ("column coordinate", matrix.col_indices()),
            ] {
                for &coordinate in coordinates {
                    let coordinate = u64::try_from(coordinate).map_err(|error| {
                        format!("{label} COO {field} does not fit u64: {error}")
                    })?;
    ...
    impl ScipyExpm {
        ...
        fn start_coo_add(script: &Path) -> Result<(Self, String), String> {
            Self::start_mode(script, "--live-coo-add")
        }
        ...
        fn initialize_coo_pair(
            ...
            .map_err(|error| format!("write INIT_COO_ADD: {error}"))?;
            self.write_usize_vector("LHS_ROWS", lhs.row_indices())?;
            self.write_usize_vector("LHS_COLS", lhs.col_indices())?;
            self.write_f64_vector("LHS_DATA", lhs.data())?;
            self.write_usize_vector("RHS_ROWS", rhs.row_indices())?;
            self.write_usize_vector("RHS_COLS", rhs.col_indices())?;
            self.write_f64_vector("RHS_DATA", rhs.data())?;
            self.stdin
                .flush()
                .map_err(|error| format!("flush INIT_COO_ADD: {error}"))?;
            self.read_line()
        }
        ...
        fn coo_add_parity(&mut self) -> Result<(String, String), String> {
            self.transpose_parity()
        }
    ...
    fn time_current_coo_add(lhs: &CooMatrix, rhs: &CooMatrix, repetitions: usize) -> f64 {
        let started = Instant::now();
        for _ in 0..repetitions {
            let result = add_coo(black_box(lhs), black_box(rhs))
                .expect("FrankenSciPy canonical COO addition");
            black_box(result);
        }
        started.elapsed().as_secs_f64()
    }
    ...
    pub fn run_coo_add_current_profile(n: usize, repetitions: usize) -> Result<(), String> {
        if n != COO_ADD_N || repetitions < 1 {
            return Err(format!(
                "registered COO-add profile requires n={COO_ADD_N} and repetitions>=1"
            ));
        }
        let (lhs, rhs) = coo_add_fixture();
        if lhs.nnz() != COO_ADD_OPERAND_NNZ
            || rhs.nnz() != COO_ADD_OPERAND_NNZ
            || !coo_is_strictly_lexicographic(&lhs)
            || !coo_is_strictly_lexicographic(&rhs)
        {
            return Err("registered COO-add operands are not canonical".to_string());
        }
        let input_sha256 = coo_pair_input_sha256(&lhs, &rhs)?;
        let result =
            add_coo(&lhs, &rhs).map_err(|error| format!("FrankenSciPy COO-add warmup: {error}"))?;
        if result.nnz() != COO_ADD_RESULT_NNZ
            || !coo_is_strictly_lexicographic(&result)
            || result
                .data()
                .iter()
                .any(|value| !value.is_finite() || *value <= 0.0)
        {
            return Err("current COO-add profile result is not canonical".to_string());
        }
        let elapsed = time_current_coo_add(&lhs, &rhs, repetitions);
        println!(
            "COO_ADD_FSCI_PROFILE n={n} entries_per_row={COO_ADD_ENTRIES_PER_ROW} \
             lhs_nnz={} rhs_nnz={} result_nnz={} repetitions={repetitions} \
             elapsed_seconds={elapsed:.9} result_format=coo sorted=true unique=true \
             input_sha256={input_sha256}",
            lhs.nnz(),
            rhs.nnz(),
            result.nnz()
    ...
    pub fn run_coo_add_vs_scipy(
        n: usize,
        rounds: usize,
        explicit_oracle: Option<&String>,
    ) -> Result<(), String> {
        if n != COO_ADD_N || rounds != COO_ADD_REGISTERED_ROUNDS {
            return Err(format!(
                "one-shot profile requires n={COO_ADD_N} rounds={COO_ADD_REGISTERED_ROUNDS}"
            ));
        }
        ...
        let (lhs, rhs) = coo_add_fixture();
        if lhs.shape() != Shape2D::new(COO_ADD_N, COO_ADD_N)
            || rhs.shape() != lhs.shape()
            || lhs.nnz() != COO_ADD_OPERAND_NNZ
            || rhs.nnz() != COO_ADD_OPERAND_NNZ
            || !coo_is_strictly_lexicographic(&lhs)
            || !coo_is_strictly_lexicographic(&rhs)
            || lhs
                ...
                .any(|value| !value.is_finite() || *value <= 0.0)
        {
            return Err("registered COO-add fixture has the wrong contract".to_string());
        }
        let input_sha256 = coo_pair_input_sha256(&lhs, &rhs)?;
        let current_coo =
            add_coo(&lhs, &rhs).map_err(|error| format!("FrankenSciPy COO-add parity: {error}"))?;
        if current_coo.shape() != lhs.shape()
            || current_coo.nnz() != COO_ADD_RESULT_NNZ
            || !coo_is_strictly_lexicographic(&current_coo)
            || current_coo
                .data()
                .iter()
                .any(|value| !value.is_finite() || *value <= 0.0)
        {
            return Err("FrankenSciPy COO-add output contract failed".to_string());
        }
        let current = current_coo
            .to_csr()
            .map_err(|error| format!("normalize FrankenSciPy COO-add result: {error}"))?;
        if current.shape() != lhs.shape()
            || current.nnz() != COO_ADD_RESULT_NNZ
            || current.indptr().len() != COO_ADD_N + 1
            || current.indptr()[0] != 0
            || current.indptr()[COO_ADD_N] != COO_ADD_RESULT_NNZ
            || !current.canonical_meta().sorted_indices
            || !current.canonical_meta().deduplicated
        {
            return Err("normalized FrankenSciPy COO-add result is not canonical".to_string());
        }
        let current_middle_pointer = current.indptr()[COO_ADD_N / 2];
        ...
        println!(
            "source_identity: harness_sha256={harness_source_sha256} ops_sha256={ops_source_sha256} \
             embedded_oracle_sha256={embedded_oracle_sha256}"
        );
        println!(
            "fixture=canonical-coo-add n={COO_ADD_N} entries_per_row={COO_ADD_ENTRIES_PER_ROW} \
             lhs_nnz={COO_ADD_OPERAND_NNZ} rhs_nnz={COO_ADD_OPERAND_NNZ} \
             result_nnz={COO_ADD_RESULT_NNZ} overlap_per_row=24 \
             column=(257*j+31*r+6168*s)%16384 \
             value=(1+((13*r+17*j+23*s)%251))/256 rounds={rounds} \
             construction_outside_timing=true serialization_outside_timing=true \
             canonical_normalization_outside_timing=true parity_outside_timing=true \
             two_arm_order=alternating null_design=four-call-forward-reverse-geometric-symmetrization \
        ...
        let (mut scipy, identity) = ScipyExpm::start_coo_add(&script)?;
        println!("scipy_arm: {identity}");
        if !identity.starts_with("READY scipy=1.17.1 ")
            || !identity.contains("method=coo_add")
            || !identity.contains("solver_mod=scipy.sparse._coo")
            || !identity.contains("actual_observed_worker_threads=1")
            || !identity.contains("fsci_loaded=False")
            || !identity.contains("genuine=True")
        {
            return Err("live SciPy COO-add failed genuine-incumbent identity gate".to_string());
        }
        let scipy_engine_sha256 = field_value(&identity, "scipy_engine_sha256=")
            .ok_or_else(|| "live SciPy omitted its COO engine SHA-256".to_string())?;
        if !is_sha256(scipy_engine_sha256) {
            return Err("live SciPy reported an invalid COO engine SHA-256".to_string());
        }
        println!("scipy_engine_sha256={scipy_engine_sha256}");

        let case = scipy.initialize_coo_pair(&lhs, &rhs)?;
        let expected_case = format!(
            "CASE method=coo_add rows={COO_ADD_N} cols={COO_ADD_N} \
             lhs_nnz={COO_ADD_OPERAND_NNZ} rhs_nnz={COO_ADD_OPERAND_NNZ} \
             lhs_sorted=True rhs_sorted=True lhs_unique=True rhs_unique=True finite=True \
             result_format=csr result_nnz={COO_ADD_RESULT_NNZ} sorted=True canonical=True"
        );
        if case != expected_case {
            return Err(format!(
                "live SciPy constructed the wrong COO-add case: {case}"
            ));
        }
        println!("scipy_case: {case}");
        let scipy_input_sha256 = scipy.input_sha256()?;
        ...
        let (live_result, live_output_sha256) = scipy.coo_add_parity()?;
        let expected_result = format!(
            "RESULT rows={COO_ADD_N} cols={COO_ADD_N} nnz={COO_ADD_RESULT_NNZ} \
             format=csr sorted=True canonical=True finite=True first_pointer=0 \
             middle_pointer={current_middle_pointer} last_pointer={COO_ADD_RESULT_NNZ}"
        );
        if live_result != expected_result || live_output_sha256 != current_output_sha256 {
            return Err(format!(
                "COO-add output contract mismatch: result={live_result} \
        ...
        println!(
            "agreement: current_output_sha256={current_output_sha256} \
             ...
             shape={COO_ADD_N}x{COO_ADD_N} nnz={COO_ADD_RESULT_NNZ} exact=true"
        );

        let quiescence_pre = sample_coo_add_quiescence("pre")?;
        let mut current_repetitions = 1usize;
        let current_calibration_seconds = loop {
            let elapsed = time_current_coo_add(&lhs, &rhs, current_repetitions);
            if elapsed >= COO_ADD_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            current_repetitions = current_repetitions
                .checked_mul(2)
                .ok_or_else(|| "current COO-add calibration overflowed".to_string())?;
        };
        let mut live_repetitions = 1usize;
        let live_calibration_seconds = loop {
            let elapsed = scipy.solve(live_repetitions, COO_ADD_RESULT_NNZ)?;
            if elapsed >= COO_ADD_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            live_repetitions = live_repetitions
                .checked_mul(2)
                .ok_or_else(|| "live COO-add calibration overflowed".to_string())?;
        };
        println!(
            "calibration: current_repetitions={current_repetitions} \
             live_repetitions={live_repetitions} min_sample_ms={} \
            ...
            COO_ADD_MIN_SAMPLE_SECONDS * 1_000.0
        );
        for _ in 0..2 {
            let _ = time_current_coo_add(&lhs, &rhs, current_repetitions);
            let _ = scipy.solve(live_repetitions, COO_ADD_RESULT_NNZ)?;
        }
        let quiescence_measurement = sample_coo_add_quiescence("measurement")?;
        ...
        for round in 0..rounds {
            let (current_batch, live_batch) = if round.is_multiple_of(2) {
                (
                    time_current_coo_add(&lhs, &rhs, current_repetitions),
                    scipy.solve(live_repetitions, COO_ADD_RESULT_NNZ)?,
                )
            } else {
                let live = scipy.solve(live_repetitions, COO_ADD_RESULT_NNZ)?;
                let current = time_current_coo_add(&lhs, &rhs, current_repetitions);
                (current, live)
            };
            let current_null = four_call_geometric_null(|| {
                Ok(time_current_coo_add(&lhs, &rhs, current_repetitions))
            })?;
            let live_null =
                four_call_geometric_null(|| scipy.solve(live_repetitions, COO_ADD_RESULT_NNZ))?;
        ...
        let quiescence_post = sample_coo_add_quiescence("post")?;
        scipy.quit();
        ...
        let profile_admitted =
            ratio_high < 0.70 && null_medians_ok && clears_null && quiescence_all_clear;
        println!(
            "timing: current_p50_ms={:.6} current_p95_ms={:.6} current_p99_ms={:.6} \
             live_scipy_p50_ms={:.6} live_scipy_p95_ms={:.6} live_scipy_p99_ms={:.6} \
             incumbent_ratio_scipy_over_frankenscipy={ratio_median:.6} \
...
fn main() {
    ...
    if mode == "coo-add-current-profile" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let n = args
                .get(2)
            ...
            let repetitions = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(1);
            if let Err(error) = expm_bench::run_coo_add_current_profile(n, repetitions) {
        ...
        {
            eprintln!("COO-add profiling requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
    if mode == "coo-add-vs-scipy" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let n = args
                .get(2)
            ...
            let rounds = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(24);
            if let Err(error) = expm_bench::run_coo_add_vs_scipy(n, rounds, args.get(4)) {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
            return;
        ...
        #[cfg(not(feature = "sparse-incumbent-bench"))]
        {
            eprintln!("COO-add comparison requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
