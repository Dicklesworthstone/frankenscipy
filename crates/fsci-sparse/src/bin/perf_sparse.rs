...
//!   `perf_sparse vstack-csr-current-profile <rows-per-block> <repeats>`
//!   `perf_sparse vstack-csr-parity <rows-per-block> [oracle]`
//!   `perf_sparse vstack-csr-vs-scipy <rows-per-block> <rounds> [oracle]`
...
mod expm_bench {
    ...
    use fsci_sparse::{
        ...
        sparse_transpose, sparse_transpose_view, vstack,
    };
    ...
    const BSR_TO_CSR_N: usize = 32_768;
    const BSR_BLOCK_SIDE: usize = 4;
    const BSR_BLOCK_ROWS: usize = BSR_TO_CSR_N / BSR_BLOCK_SIDE;
    const BSR_BLOCKS_PER_ROW: usize = 24;
    ...
    const LIL_SKEW_ROWS: usize = 65_536;
    const LIL_SKEW_COLS: usize = 262_144;
    const LIL_SKEW_WIDTHS: [usize; 8] = [0, 1, 3, 7, 15, 31, 63, 127];
    const LIL_SKEW_NNZ: usize = (LIL_SKEW_ROWS / LIL_SKEW_WIDTHS.len()) * 247;
    const LIL_SKEW_REGISTERED_ROUNDS: usize = 24;
    ...
    const VSTACK_BLOCK_COUNT: usize = 8;
    const VSTACK_ROWS_PER_BLOCK: usize = 32_768;
    const VSTACK_COLS: usize = 65_536;
    const VSTACK_ENTRIES_PER_ROW: usize = 8;
    const VSTACK_BLOCK_NNZ: usize = VSTACK_ROWS_PER_BLOCK * VSTACK_ENTRIES_PER_ROW;
    const VSTACK_RESULT_ROWS: usize = VSTACK_BLOCK_COUNT * VSTACK_ROWS_PER_BLOCK;
    const VSTACK_RESULT_NNZ: usize = VSTACK_BLOCK_COUNT * VSTACK_BLOCK_NNZ;
    const VSTACK_REGISTERED_ROUNDS: usize = 24;
    const VSTACK_MIN_SAMPLE_SECONDS: f64 = 0.050;
    const VSTACK_EMBEDDED_SOURCE_COMMIT: Option<&str> = option_env!("FSCI_EMBEDDED_SOURCE_COMMIT");
    ...
    const BSR_TO_CSR_MIN_SAMPLE_SECONDS: f64 = 0.050;
    ...
    fn bsr_to_csr_fixture() -> BsrMatrix {
        let block_shape = Shape2D::new(BSR_BLOCK_SIDE, BSR_BLOCK_SIDE);
        let block_area = BSR_BLOCK_SIDE * BSR_BLOCK_SIDE;
        let mut data = Vec::with_capacity(BSR_STORED_BLOCKS);
        let mut indices = Vec::with_capacity(BSR_STORED_BLOCKS);
        let mut indptr = Vec::with_capacity(BSR_BLOCK_ROWS + 1);
        indptr.push(0);
        for block_row in 0..BSR_BLOCK_ROWS {
            let mut entries = (0..BSR_BLOCKS_PER_ROW)
                .map(|slot| ((257 * slot + 31 * block_row) % BSR_BLOCK_ROWS, slot))
                .collect::<Vec<_>>();
            entries.sort_unstable_by_key(|entry| entry.0);
            for (block_column, slot) in entries {
                let block = (0..block_area)
                    .map(|offset| {
                        (1 + ((13 * block_row + 17 * slot + 19 * offset) % 251)) as f64 / 256.0
                    })
                    .collect::<Vec<_>>();
        ...
        BsrMatrix::from_components(
            Shape2D::new(BSR_TO_CSR_N, BSR_TO_CSR_N),
            block_shape,
            data,
            indices,
            indptr,
    ...
    fn lil_skew_to_csr_fixture() -> LilMatrix {
        let mut row_indices = Vec::with_capacity(LIL_SKEW_ROWS);
        let mut row_data = Vec::with_capacity(LIL_SKEW_ROWS);
        for row in 0..LIL_SKEW_ROWS {
            let width = LIL_SKEW_WIDTHS[row % LIL_SKEW_WIDTHS.len()];
            let mut entries = (0..width)
                .map(|slot| ((4_093 * slot + 97 * row) % LIL_SKEW_COLS, slot))
                .collect::<Vec<_>>();
            entries.sort_unstable_by_key(|entry| entry.0);
            ...
            for (column, slot) in entries {
                columns.push(column);
                values.push((1 + ((29 * row + 37 * slot) % 997)) as f64 / 1_024.0);
            }
            row_indices.push(columns);
            row_data.push(values);
        }
        LilMatrix::from_rows(
            Shape2D::new(LIL_SKEW_ROWS, LIL_SKEW_COLS),
            row_indices,
            row_data,
        )
    ...
    fn vstack_csr_fixture() -> Vec<CsrMatrix> {
        (0..VSTACK_BLOCK_COUNT)
            .map(|block| {
                let mut data = Vec::with_capacity(VSTACK_BLOCK_NNZ);
                let mut indices = Vec::with_capacity(VSTACK_BLOCK_NNZ);
                let mut indptr = Vec::with_capacity(VSTACK_ROWS_PER_BLOCK + 1);
                indptr.push(0);
                for row in 0..VSTACK_ROWS_PER_BLOCK {
                    let mut entries = (0..VSTACK_ENTRIES_PER_ROW)
                        .map(|slot| ((4_099 * slot + 73 * row + 211 * block) % VSTACK_COLS, slot))
                        .collect::<Vec<_>>();
                    entries.sort_unstable_by_key(|entry| entry.0);
                    for (column, slot) in entries {
                        indices.push(column);
                ...
                CsrMatrix::from_components(
                    Shape2D::new(VSTACK_ROWS_PER_BLOCK, VSTACK_COLS),
                    data,
                    indices,
                    indptr,
                    false,
                )
                .expect("canonical CSR vstack fixture block")
            })
            .collect()
    }

    fn vstack_input_sha256(blocks: &[CsrMatrix]) -> Result<String, String> {
        hstack_input_sha256(blocks)
    }
    ...
    fn lil_input_sha256(matrix: &LilMatrix) -> Result<String, String> {
        ...
        for (label, value) in [
            ("row count", matrix.shape().rows),
            ("column count", matrix.shape().cols),
            ("stored value count", matrix.nnz()),
        ] {
            let value = u64::try_from(value)
                .map_err(|error| format!("{label} does not fit u64: {error}"))?;
            digest.update(value.to_le_bytes());
        }
        for row in matrix.row_data() {
            for &value in row {
                digest.update(value.to_le_bytes());
            }
        }
        for row in matrix.row_indices() {
            for &column in row {
                let column = u64::try_from(column)
                    .map_err(|error| format!("LIL column does not fit u64: {error}"))?;
        ...
        for row in matrix.row_data() {
            pointer = pointer
                .checked_add(row.len())
                .ok_or_else(|| "LIL input pointer overflowed".to_string())?;
            let pointer = u64::try_from(pointer)
                .map_err(|error| format!("LIL pointer does not fit u64: {error}"))?;
    ...
    fn bsr_input_sha256(matrix: &BsrMatrix) -> Result<String, String> {
        ...
        for (label, value) in [
            ("row count", matrix.shape().rows),
            ("column count", matrix.shape().cols),
            ("block row size", matrix.block_shape().rows),
            ("block column size", matrix.block_shape().cols),
            ("stored block count", matrix.nnz_blocks()),
        ] {
            let value = u64::try_from(value)
                .map_err(|error| format!("{label} does not fit u64: {error}"))?;
            digest.update(value.to_le_bytes());
        }
        for block in matrix.data() {
            for &value in block {
                digest.update(value.to_le_bytes());
            }
        }
        for (label, values) in [
            ("block column index", matrix.indices()),
            ("block row pointer", matrix.indptr()),
        ] {
            for &value in values {
                let value = u64::try_from(value)
                    .map_err(|error| format!("{label} does not fit u64: {error}"))?;
    ...
    fn sample_registered_lil_quiescence(phase: &str) -> Result<bool, String> {
    ...
    impl ScipyExpm {
        ...
        fn start_bsr_to_csr(script: &Path) -> Result<(Self, String), String> {
            Self::start_mode(script, "--live-bsr-to-csr")
        }

        fn start_lil_skew_to_csr(script: &Path) -> Result<(Self, String), String> {
        ...
        fn start_vstack_csr(script: &Path) -> Result<(Self, String), String> {
            Self::start_mode(script, "--live-vstack-csr")
        }
        ...
        fn initialize_bsr(&mut self, matrix: &BsrMatrix) -> Result<String, String> {
            writeln!(
                self.stdin,
                "INIT_BSR {} {} {} {} {}",
                matrix.shape().rows,
            ...
            .map_err(|error| format!("write INIT_BSR: {error}"))?;
            self.write_usize_vector("BSR_INDPTR", matrix.indptr())?;
            self.write_usize_vector("BSR_INDICES", matrix.indices())?;
            let flattened = matrix
                .data()
                .iter()
                .flat_map(|block| block.iter().copied())
                .collect::<Vec<_>>();
            self.write_f64_vector("BSR_DATA", &flattened)?;
            self.stdin
                .flush()
                .map_err(|error| format!("flush INIT_BSR: {error}"))?;
            self.read_line()
        }

        fn initialize_lil(&mut self, matrix: &LilMatrix) -> Result<String, String> {
            writeln!(
                self.stdin,
                "INIT_LIL {} {} {}",
                matrix.shape().rows,
            ...
            .map_err(|error| format!("write INIT_LIL: {error}"))?;
            let mut indptr = Vec::with_capacity(matrix.shape().rows + 1);
            let mut flat_indices = Vec::with_capacity(matrix.nnz());
            let mut flat_data = Vec::with_capacity(matrix.nnz());
            indptr.push(0usize);
            for (columns, values) in matrix.row_indices().iter().zip(matrix.row_data()) {
                flat_indices.extend_from_slice(columns);
                flat_data.extend_from_slice(values);
                indptr.push(flat_indices.len());
            }
            self.write_usize_vector("LIL_INDPTR", &indptr)?;
            self.write_usize_vector("LIL_INDICES", &flat_indices)?;
            self.write_f64_vector("LIL_DATA", &flat_data)?;
            self.stdin
                .flush()
                .map_err(|error| format!("flush INIT_LIL: {error}"))?;
            self.read_line()
        }
        ...
        fn initialize_vstack(&mut self, blocks: &[CsrMatrix]) -> Result<String, String> {
            writeln!(
                self.stdin,
                "INIT_VSTACK {} {} {} {}",
                blocks.len(),
                VSTACK_ROWS_PER_BLOCK,
                VSTACK_COLS,
                VSTACK_BLOCK_NNZ
            )
            .map_err(|error| format!("write INIT_VSTACK: {error}"))?;
            for (block_index, matrix) in blocks.iter().enumerate() {
                writeln!(
                    self.stdin,
                    "BLOCK {block_index} {} {} {}",
                ...
                .map_err(|error| format!("write vstack block {block_index}: {error}"))?;
                self.write_usize_vector("VSTACK_INDPTR", matrix.indptr())?;
                self.write_usize_vector("VSTACK_INDICES", matrix.indices())?;
                self.write_f64_vector("VSTACK_DATA", matrix.data())?;
            }
            self.stdin
                .flush()
                .map_err(|error| format!("flush INIT_VSTACK: {error}"))?;
            self.read_line()
        }
        ...
        fn bsr_to_csr_parity(&mut self) -> Result<(String, String), String> {
        ...
        fn vstack_parity(&mut self) -> Result<(String, String), String> {
            self.transpose_parity()
        }
    ...
    fn time_bsr_to_csr(matrix: &BsrMatrix, repetitions: usize) -> f64 {
        let mut checksum = 0usize;
        let started = Instant::now();
        for _ in 0..repetitions {
            let result = matrix
                .to_csr()
                .expect("FrankenSciPy canonical BSR-to-CSR conversion");
            checksum ^= result
                .nnz()
                .wrapping_add(result.shape().rows)
                .wrapping_add(result.shape().cols);
    ...
    fn time_lil_skew_to_csr(matrix: &LilMatrix, repetitions: usize) -> f64 {
        let mut checksum = 0usize;
        let started = Instant::now();
        for _ in 0..repetitions {
            let result = matrix
    ...
    fn time_vstack_csr(blocks: &[&dyn FormatConvertible], repetitions: usize) -> f64 {
        let mut checksum = 0usize;
        let started = Instant::now();
        for _ in 0..repetitions {
            let result = vstack(black_box(blocks)).expect("FrankenSciPy canonical CSR vstack");
            checksum ^= result
                .nnz()
                .wrapping_add(result.shape().rows)
                .wrapping_add(result.shape().cols);
    ...
    pub fn run_bsr_to_csr_current_profile(n: usize, repetitions: usize) -> Result<(), String> {
        if n != BSR_TO_CSR_N || repetitions < 1 {
            return Err(format!(
                "registered BSR-to-CSR profile requires n={BSR_TO_CSR_N} and repetitions>=1"
            ));
        }
        ...
        let result = matrix
            .to_csr()
            .map_err(|error| format!("FrankenSciPy BSR-to-CSR warmup: {error}"))?;
        if result.nnz() != BSR_SCALAR_NNZ
            || !result.canonical_meta().sorted_indices
            || !result.canonical_meta().deduplicated
        {
            return Err("current BSR-to-CSR profile result is not canonical".to_string());
        }
        let elapsed = time_bsr_to_csr(&matrix, repetitions);
        println!(
            "BSR_TO_CSR_FSCI_PROFILE n={n} block_side={BSR_BLOCK_SIDE} \
             block_rows={BSR_BLOCK_ROWS} blocks_per_row={BSR_BLOCKS_PER_ROW} \
             stored_blocks={BSR_STORED_BLOCKS} scalar_nnz={BSR_SCALAR_NNZ} \
             repetitions={repetitions} elapsed_seconds={elapsed:.9} \
             result_format=csr result_nnz={} input_sha256={input_sha256}",
            result.nnz()
        );
        Ok(())
    }

    pub fn run_bsr_to_csr_vs_scipy(
        n: usize,
        rounds: usize,
        explicit_oracle: Option<&String>,
    ) -> Result<(), String> {
        if n != BSR_TO_CSR_N || rounds != BSR_TO_CSR_REGISTERED_ROUNDS {
            return Err(format!(
                "one-shot profile requires n={BSR_TO_CSR_N} \
                 rounds={BSR_TO_CSR_REGISTERED_ROUNDS}"
            ));
        }
        ...
        if matrix.shape() != Shape2D::new(BSR_TO_CSR_N, BSR_TO_CSR_N)
            || matrix.block_shape() != Shape2D::new(BSR_BLOCK_SIDE, BSR_BLOCK_SIDE)
            || matrix.nnz_blocks() != BSR_STORED_BLOCKS
            || matrix.nnz() != BSR_SCALAR_NNZ
            || !matrix.canonical_meta().sorted_indices
            || !matrix.canonical_meta().deduplicated
        {
            return Err("registered BSR fixture has the wrong canonical contract".to_string());
        }
        let input_sha256 = bsr_input_sha256(&matrix)?;
        let current = matrix
            .to_csr()
            .map_err(|error| format!("FrankenSciPy BSR-to-CSR parity: {error}"))?;
        if current.shape() != matrix.shape()
            || current.nnz() != BSR_SCALAR_NNZ
            || current.indptr().len() != BSR_TO_CSR_N + 1
            || current.indptr()[0] != 0
            || current.indptr()[BSR_TO_CSR_N / 2] != BSR_SCALAR_NNZ / 2
            || current.indptr()[BSR_TO_CSR_N] != BSR_SCALAR_NNZ
            || current.data().iter().any(|value| !value.is_finite())
            || !current.canonical_meta().sorted_indices
            || !current.canonical_meta().deduplicated
        {
            return Err("FrankenSciPy BSR-to-CSR output contract failed".to_string());
        }
        ...
        println!(
            "fixture=canonical-bsr-to-csr n={BSR_TO_CSR_N} block_side={BSR_BLOCK_SIDE} \
             block_rows={BSR_BLOCK_ROWS} blocks_per_row={BSR_BLOCKS_PER_ROW} \
             stored_blocks={BSR_STORED_BLOCKS} scalar_nnz={BSR_SCALAR_NNZ} \
             block_column=(257*j+31*r)%8192 \
             value=(1+((13*r+17*j+19*q)%251))/256 rounds={rounds} \
             construction_outside_timing=true serialization_outside_timing=true \
        ...
        if !identity.starts_with("READY scipy=1.17.1 ")
            ...
            || !identity.contains("genuine=True")
        {
            return Err("live SciPy BSR-to-CSR failed genuine-incumbent identity gate".to_string());
        }
        let scipy_engine_sha256 = field_value(&identity, "scipy_engine_sha256=")
        ...
        let expected_case = format!(
            "CASE method=bsr_to_csr rows={BSR_TO_CSR_N} cols={BSR_TO_CSR_N} \
             block_rows={BSR_BLOCK_SIDE} block_cols={BSR_BLOCK_SIDE} \
             stored_blocks={BSR_STORED_BLOCKS} scalar_nnz={BSR_SCALAR_NNZ} \
             sorted=True canonical=True finite=True result_format=csr \
             result_nnz={BSR_SCALAR_NNZ}"
        );
        if case != expected_case {
        ...
        let expected_result = format!(
            "RESULT rows={BSR_TO_CSR_N} cols={BSR_TO_CSR_N} nnz={BSR_SCALAR_NNZ} \
             format=csr sorted=True canonical=True finite=True \
             first_pointer=0 middle_pointer={} last_pointer={BSR_SCALAR_NNZ}",
            BSR_SCALAR_NNZ / 2
        );
        if live_result != expected_result || live_output_sha256 != current_output_sha256 {
            return Err(format!(
                "BSR-to-CSR output contract mismatch: result={live_result} \
        ...
        println!(
            "agreement: current_output_sha256={current_output_sha256} \
             ...
             shape={BSR_TO_CSR_N}x{BSR_TO_CSR_N} nnz={BSR_SCALAR_NNZ} exact=true"
        );

        let quiescence_pre = sample_host_wide_quiescence("pre")?;
        let mut current_repetitions = 1usize;
        let current_calibration_seconds = loop {
            let elapsed = time_bsr_to_csr(&matrix, current_repetitions);
            if elapsed >= BSR_TO_CSR_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            current_repetitions = current_repetitions
                .checked_mul(2)
                .ok_or_else(|| "current BSR-to-CSR calibration overflowed".to_string())?;
        };
        let mut live_repetitions = 1usize;
        let live_calibration_seconds = loop {
            let elapsed = scipy.solve(live_repetitions, BSR_SCALAR_NNZ)?;
            if elapsed >= BSR_TO_CSR_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            live_repetitions = live_repetitions
                .checked_mul(2)
                .ok_or_else(|| "live BSR-to-CSR calibration overflowed".to_string())?;
        };
        println!(
            "calibration: current_repetitions={current_repetitions} \
             live_repetitions={live_repetitions} min_sample_ms={} \
        ...
        for _ in 0..2 {
            let _ = time_bsr_to_csr(&matrix, current_repetitions);
            let _ = scipy.solve(live_repetitions, BSR_SCALAR_NNZ)?;
        }
        ...
        for round in 0..rounds {
            let (current_batch, live_batch) = if round.is_multiple_of(2) {
                (
                    time_bsr_to_csr(&matrix, current_repetitions),
                    scipy.solve(live_repetitions, BSR_SCALAR_NNZ)?,
                )
            } else {
                let live = scipy.solve(live_repetitions, BSR_SCALAR_NNZ)?;
                let current = time_bsr_to_csr(&matrix, current_repetitions);
                (current, live)
            };
            let current_null =
                four_call_geometric_null(|| Ok(time_bsr_to_csr(&matrix, current_repetitions)))?;
            let live_null =
                four_call_geometric_null(|| scipy.solve(live_repetitions, BSR_SCALAR_NNZ))?;
        ...
        let profile_admitted =
            ratio_high < 0.80 && null_medians_ok && clears_null && quiescence_all_clear;
        println!(
            "timing: current_p50_ms={:.6} current_p95_ms={:.6} current_p99_ms={:.6} \
             live_scipy_p50_ms={:.6} live_scipy_p95_ms={:.6} live_scipy_p99_ms={:.6} \
             incumbent_ratio_scipy_over_frankenscipy={ratio_median:.6} \
    ...
    pub fn run_lil_skew_to_csr_current_profile(
        rows: usize,
        repetitions: usize,
    ) -> Result<(), String> {
        if rows != LIL_SKEW_ROWS || repetitions < 1 {
            return Err(format!(
                "registered skewed LIL-to-CSR profile requires rows={LIL_SKEW_ROWS} \
                 and repetitions>=1"
            ));
        }
        let matrix = lil_skew_to_csr_fixture();
        let input_sha256 = lil_input_sha256(&matrix)?;
        let result = matrix
            .to_csr()
            .map_err(|error| format!("FrankenSciPy skewed LIL-to-CSR warmup: {error}"))?;
        if result.shape() != Shape2D::new(LIL_SKEW_ROWS, LIL_SKEW_COLS)
            || result.nnz() != LIL_SKEW_NNZ
            || !result.canonical_meta().sorted_indices
            || !result.canonical_meta().deduplicated
        {
            return Err("current skewed LIL-to-CSR profile result is not canonical".to_string());
        }
        let elapsed = time_lil_skew_to_csr(&matrix, repetitions);
        println!(
            "LIL_SKEW_TO_CSR_FSCI_PROFILE rows={rows} cols={LIL_SKEW_COLS} \
             widths=0,1,3,7,15,31,63,127 nnz={LIL_SKEW_NNZ} \
             repetitions={repetitions} elapsed_seconds={elapsed:.9} \
             result_format=csr result_nnz={} input_sha256={input_sha256}",
            result.nnz()
        );
    ...
    pub fn run_lil_skew_to_csr_vs_scipy(
        rows: usize,
        rounds: usize,
        explicit_oracle: Option<&String>,
    ) -> Result<(), String> {
        if rows != LIL_SKEW_ROWS || rounds != LIL_SKEW_REGISTERED_ROUNDS {
            return Err(format!(
                "one-shot profile requires rows={LIL_SKEW_ROWS} \
                 rounds={LIL_SKEW_REGISTERED_ROUNDS}"
            ));
        }
        let runtime_source_commit = required_env("BINARY_SOURCE_COMMIT")?;
        let embedded_source_commit = LIL_SKEW_EMBEDDED_SOURCE_COMMIT.ok_or_else(|| {
        ...
        let matrix = lil_skew_to_csr_fixture();
        if matrix.shape() != Shape2D::new(LIL_SKEW_ROWS, LIL_SKEW_COLS)
            || matrix.nnz() != LIL_SKEW_NNZ
            || matrix
                .row_indices()
                .iter()
                .zip(matrix.row_data())
        ...
        {
            return Err("registered skewed LIL fixture has the wrong contract".to_string());
        }
        let input_sha256 = lil_input_sha256(&matrix)?;
        let current = matrix
            .to_csr()
            .map_err(|error| format!("FrankenSciPy skewed LIL-to-CSR parity: {error}"))?;
        if current.shape() != matrix.shape()
            || current.nnz() != LIL_SKEW_NNZ
            || current.indptr().len() != LIL_SKEW_ROWS + 1
            || current.indptr()[0] != 0
            || current.indptr()[LIL_SKEW_ROWS / 2] != LIL_SKEW_NNZ / 2
            || current.indptr()[LIL_SKEW_ROWS] != LIL_SKEW_NNZ
            || current
                .data()
                .iter()
                .any(|value| !value.is_finite() || *value <= 0.0)
            ...
            || !current.canonical_meta().deduplicated
        {
            return Err("FrankenSciPy skewed LIL-to-CSR output contract failed".to_string());
        }
        let current_output_sha256 = compressed_parts_sha256(
            current.shape(),
            current.data(),
        ...
        println!(
            "fixture=rectangular-skewed-lil-to-csr rows={LIL_SKEW_ROWS} \
             cols={LIL_SKEW_COLS} widths=0,1,3,7,15,31,63,127 nnz={LIL_SKEW_NNZ} \
             column=(4093*j+97*r)%262144 value=(1+((29*r+37*j)%997))/1024 \
             rounds={rounds} construction_outside_timing=true \
             serialization_outside_timing=true parity_outside_timing=true \
             two_arm_order=alternating \
             null_design=four-call-forward-reverse-geometric-symmetrization \
             same_invocation=true side_by_side=true"
        );
        ...
        if !identity.starts_with("READY scipy=1.17.1 ")
            ...
            || !identity.contains("genuine=True")
        {
            return Err("live SciPy skewed LIL-to-CSR identity gate failed".to_string());
        }
        let scipy_engine_sha256 = field_value(&identity, "scipy_engine_sha256=")
        ...
        let expected_case = format!(
            "CASE method=lil_skew_to_csr rows={LIL_SKEW_ROWS} cols={LIL_SKEW_COLS} \
             nnz={LIL_SKEW_NNZ} min_row_nnz=0 max_row_nnz=127 empty_rows=8192 \
             sorted=True canonical=True finite=True result_format=csr \
             result_nnz={LIL_SKEW_NNZ}"
        );
        if case != expected_case {
            return Err(format!("live SciPy constructed the wrong LIL case: {case}"));
        }
        println!("scipy_case: {case}");
        let scipy_input_sha256 = scipy.input_sha256()?;
        if input_sha256 != scipy_input_sha256 {
        ...
        let expected_result = format!(
            "RESULT rows={LIL_SKEW_ROWS} cols={LIL_SKEW_COLS} nnz={LIL_SKEW_NNZ} \
             format=csr sorted=True canonical=True finite=True first_pointer=0 \
             middle_pointer={} last_pointer={LIL_SKEW_NNZ}",
            LIL_SKEW_NNZ / 2
        );
        if live_result != expected_result || live_output_sha256 != current_output_sha256 {
            return Err(format!(
                "skewed LIL-to-CSR output mismatch: result={live_result} \
                 current_sha256={current_output_sha256} live_sha256={live_output_sha256}"
            ));
        }
        println!(
        ...
        println!(
            "agreement: current_output_sha256={current_output_sha256} \
             scipy_output_sha256={live_output_sha256} output_digest_match=true \
             shape={LIL_SKEW_ROWS}x{LIL_SKEW_COLS} nnz={LIL_SKEW_NNZ} exact=true"
        );

        let quiescence_pre = sample_registered_lil_quiescence("pre")?;
        let mut current_repetitions = 1usize;
        let current_calibration_seconds = loop {
            let elapsed = time_lil_skew_to_csr(&matrix, current_repetitions);
            if elapsed >= LIL_SKEW_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            current_repetitions = current_repetitions
                .checked_mul(2)
                .ok_or_else(|| "current skewed LIL calibration overflowed".to_string())?;
        };
        let mut live_repetitions = 1usize;
        let live_calibration_seconds = loop {
            let elapsed = scipy.solve(live_repetitions, LIL_SKEW_NNZ)?;
            if elapsed >= LIL_SKEW_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            live_repetitions = live_repetitions
                .checked_mul(2)
                .ok_or_else(|| "live skewed LIL calibration overflowed".to_string())?;
        };
        let duration_pass = current_calibration_seconds >= LIL_SKEW_MIN_SAMPLE_SECONDS
            && live_calibration_seconds >= LIL_SKEW_MIN_SAMPLE_SECONDS;
        println!(
            "calibration: current_repetitions={current_repetitions} \
        ...
        for _ in 0..2 {
            let _ = time_lil_skew_to_csr(&matrix, current_repetitions);
            let _ = scipy.solve(live_repetitions, LIL_SKEW_NNZ)?;
        }
        ...
        for round in 0..rounds {
            let (current_batch, live_batch) = if round.is_multiple_of(2) {
                (
                    time_lil_skew_to_csr(&matrix, current_repetitions),
                    scipy.solve(live_repetitions, LIL_SKEW_NNZ)?,
                )
            } else {
                let live = scipy.solve(live_repetitions, LIL_SKEW_NNZ)?;
                let current = time_lil_skew_to_csr(&matrix, current_repetitions);
                (current, live)
            };
            let current_null = four_call_geometric_null(|| {
                Ok(time_lil_skew_to_csr(&matrix, current_repetitions))
            })?;
            let live_null =
                four_call_geometric_null(|| scipy.solve(live_repetitions, LIL_SKEW_NNZ))?;
            let current_seconds = current_batch / current_repetitions as f64;
            let live_seconds = live_batch / live_repetitions as f64;
            current_times.push(current_seconds);
            live_times.push(live_seconds);
        ...
        let clears_null =
            effect_deviation > 2.0 * null_half_width && effect_deviation > 2.0 * (null_edge - 1.0);
        let quiescence_all_clear = quiescence_pre && quiescence_measurement && quiescence_post;
        let profile_admitted = ratio_high < 0.80
            && null_medians_ok
            && clears_null
            && duration_pass
    ...
    fn validate_vstack_fixture(blocks: &[CsrMatrix]) -> Result<(), String> {
        if blocks.len() != VSTACK_BLOCK_COUNT {
            return Err(format!(
                "registered vstack fixture has {} blocks, expected {VSTACK_BLOCK_COUNT}",
                blocks.len()
            ));
        }
        for (block_index, block) in blocks.iter().enumerate() {
            if block.shape() != Shape2D::new(VSTACK_ROWS_PER_BLOCK, VSTACK_COLS)
                || block.nnz() != VSTACK_BLOCK_NNZ
                || block.indptr().len() != VSTACK_ROWS_PER_BLOCK + 1
                || block.indptr()[0] != 0
                || block.indptr()[VSTACK_ROWS_PER_BLOCK] != VSTACK_BLOCK_NNZ
                || !block.canonical_meta().sorted_indices
                || !block.canonical_meta().deduplicated
                || block
                    .data()
                    .iter()
                    .any(|value| !value.is_finite() || *value <= 0.0)
                || (0..VSTACK_ROWS_PER_BLOCK).any(|row| {
                    let start = block.indptr()[row];
                    let end = block.indptr()[row + 1];
                    end - start != VSTACK_ENTRIES_PER_ROW
                        || !block.indices()[start..end]
                            .windows(2)
                            .all(|pair| pair[0] < pair[1])
                })
            {
                return Err(format!(
                    "registered vstack fixture block {block_index} has the wrong contract"
                ));
            }
        }
        Ok(())
    ...
    fn validate_vstack_output(result: &CsrMatrix) -> Result<(), String> {
        if result.shape() != Shape2D::new(VSTACK_RESULT_ROWS, VSTACK_COLS)
            || result.nnz() != VSTACK_RESULT_NNZ
            || result.indptr().len() != VSTACK_RESULT_ROWS + 1
            || result.indptr()[0] != 0
            || result.indptr()[VSTACK_RESULT_ROWS / 2] != VSTACK_RESULT_NNZ / 2
            || result.indptr()[VSTACK_RESULT_ROWS] != VSTACK_RESULT_NNZ
            || !result.canonical_meta().sorted_indices
            || !result.canonical_meta().deduplicated
            || result
                .data()
                ...
                .any(|value| !value.is_finite() || *value <= 0.0)
        {
            return Err("FrankenSciPy canonical CSR vstack output contract failed".to_string());
        }
        Ok(())
    }

    fn initialize_live_vstack(
        script: &Path,
        blocks: &[CsrMatrix],
        input_sha256: &str,
        current_output_sha256: &str,
    ) -> Result<(ScipyExpm, String, String), String> {
        let (mut scipy, identity) = ScipyExpm::start_vstack_csr(script)?;
        if !identity.starts_with("READY scipy=1.17.1 ")
            || !identity.contains("method=vstack_csr")
            || !identity.contains("solver_mod=scipy.sparse._construct")
            || !identity.contains("actual_observed_worker_threads=1")
            || !identity.contains("fsci_loaded=False")
            || !identity.contains("genuine=True")
        {
            return Err("live SciPy vstack identity gate failed".to_string());
        }
        let scipy_engine_sha256 = field_value(&identity, "scipy_engine_sha256=")
            .ok_or_else(|| "live SciPy omitted its vstack engine SHA-256".to_string())?
            .to_string();
        if !is_sha256(&scipy_engine_sha256) {
            return Err("live SciPy reported an invalid vstack engine SHA-256".to_string());
        }
        let case = scipy.initialize_vstack(blocks)?;
        let expected_case = format!(
            "CASE method=vstack_csr blocks={VSTACK_BLOCK_COUNT} \
             block_rows={VSTACK_ROWS_PER_BLOCK} block_cols={VSTACK_COLS} \
             block_nnz={VSTACK_BLOCK_NNZ} output_rows={VSTACK_RESULT_ROWS} \
             output_cols={VSTACK_COLS} output_nnz={VSTACK_RESULT_NNZ} \
             sorted=True canonical=True finite=True result_format=csr"
        );
        if case != expected_case {
            return Err(format!(
                "live SciPy constructed the wrong vstack case: {case}"
            ));
        }
        let scipy_input_sha256 = scipy.input_sha256()?;
        if scipy_input_sha256 != input_sha256 {
            return Err(format!(
                "vstack input digest mismatch: frankenscipy={input_sha256} \
                 scipy={scipy_input_sha256}"
            ));
        }
        let (live_result, live_output_sha256) = scipy.vstack_parity()?;
        let expected_result = format!(
            "RESULT rows={VSTACK_RESULT_ROWS} cols={VSTACK_COLS} nnz={VSTACK_RESULT_NNZ} \
             format=csr sorted=True canonical=True finite=True first_pointer=0 \
             middle_pointer={} last_pointer={VSTACK_RESULT_NNZ}",
            VSTACK_RESULT_NNZ / 2
        );
        if live_result != expected_result || live_output_sha256 != current_output_sha256 {
            return Err(format!(
                "vstack output mismatch: result={live_result} \
                 current_sha256={current_output_sha256} live_sha256={live_output_sha256}"
            ));
        }
        Ok((scipy, identity, scipy_engine_sha256))
    ...
    pub fn run_vstack_csr_current_profile(
        rows_per_block: usize,
        repetitions: usize,
    ) -> Result<(), String> {
        if rows_per_block != VSTACK_ROWS_PER_BLOCK || repetitions < 1 {
            return Err(format!(
                "registered canonical CSR vstack profile requires \
                 rows_per_block={VSTACK_ROWS_PER_BLOCK} and repetitions>=1"
            ));
        }
        let blocks = vstack_csr_fixture();
        validate_vstack_fixture(&blocks)?;
        let input_sha256 = vstack_input_sha256(&blocks)?;
        let block_refs = hstack_block_refs(&blocks);
        let result = vstack(&block_refs)
            .map_err(|error| format!("FrankenSciPy canonical CSR vstack warmup: {error}"))?;
        validate_vstack_output(&result)?;
        let elapsed = time_vstack_csr(&block_refs, repetitions);
        println!(
            "VSTACK_CSR_FSCI_PROFILE blocks={VSTACK_BLOCK_COUNT} \
             rows_per_block={rows_per_block} cols={VSTACK_COLS} \
             entries_per_row={VSTACK_ENTRIES_PER_ROW} block_nnz={VSTACK_BLOCK_NNZ} \
             output_rows={VSTACK_RESULT_ROWS} output_nnz={VSTACK_RESULT_NNZ} \
             repetitions={repetitions} elapsed_seconds={elapsed:.9} \
             result_format=csr result_nnz={} input_sha256={input_sha256}",
            result.nnz()
        );
    ...
    pub fn run_vstack_csr_parity(
        rows_per_block: usize,
        explicit_oracle: Option<&String>,
    ) -> Result<(), String> {
        if rows_per_block != VSTACK_ROWS_PER_BLOCK {
            return Err(format!(
                "canonical CSR vstack parity requires \
                 rows_per_block={VSTACK_ROWS_PER_BLOCK}"
            ));
        }
        let blocks = vstack_csr_fixture();
        validate_vstack_fixture(&blocks)?;
        let block_refs = hstack_block_refs(&blocks);
        let input_sha256 = vstack_input_sha256(&blocks)?;
        let current = vstack(&block_refs)
            .map_err(|error| format!("FrankenSciPy canonical CSR vstack parity: {error}"))?;
        validate_vstack_output(&current)?;
        let output_sha256 = compressed_parts_sha256(
            current.shape(),
            current.data(),
            current.indices(),
        ...
        let script = oracle_path(explicit_oracle)?;
        let (scipy, identity, scipy_engine_sha256) =
            initialize_live_vstack(&script, &blocks, &input_sha256, &output_sha256)?;
        scipy.quit();
        println!(
            "VSTACK_CSR_PARITY blocks={VSTACK_BLOCK_COUNT} \
             rows_per_block={VSTACK_ROWS_PER_BLOCK} cols={VSTACK_COLS} \
             block_nnz={VSTACK_BLOCK_NNZ} output_rows={VSTACK_RESULT_ROWS} \
             output_nnz={VSTACK_RESULT_NNZ} input_sha256={input_sha256} \
             current_output_sha256={output_sha256} scipy_output_sha256={output_sha256} \
             scipy_engine_sha256={scipy_engine_sha256} exact=true identity=\"{identity}\""
        );
        Ok(())
    }

    pub fn run_vstack_csr_vs_scipy(
        rows_per_block: usize,
        rounds: usize,
        explicit_oracle: Option<&String>,
    ) -> Result<(), String> {
        if rows_per_block != VSTACK_ROWS_PER_BLOCK || rounds != VSTACK_REGISTERED_ROUNDS {
            return Err(format!(
                "one-shot vstack profile requires rows_per_block={VSTACK_ROWS_PER_BLOCK} \
                 rounds={VSTACK_REGISTERED_ROUNDS}"
            ));
        }
        let runtime_source_commit = required_env("BINARY_SOURCE_COMMIT")?;
        let embedded_source_commit = VSTACK_EMBEDDED_SOURCE_COMMIT.ok_or_else(|| {
            "FSCI_EMBEDDED_SOURCE_COMMIT was not supplied to the build".to_string()
        })?;
        if runtime_source_commit != embedded_source_commit {
            return Err(format!(
        ...
        println!(
            "source_identity: harness_sha256={harness_source_sha256} \
             construct_sha256={construct_source_sha256} formats_sha256={formats_source_sha256} \
             ops_sha256={ops_source_sha256} embedded_oracle_sha256={embedded_oracle_sha256}"
        );
        println!(
            "fixture=canonical-csr-vstack blocks={VSTACK_BLOCK_COUNT} \
             rows_per_block={VSTACK_ROWS_PER_BLOCK} cols={VSTACK_COLS} \
             entries_per_row={VSTACK_ENTRIES_PER_ROW} block_nnz={VSTACK_BLOCK_NNZ} \
             output_rows={VSTACK_RESULT_ROWS} output_nnz={VSTACK_RESULT_NNZ} \
             column=(4099*j+73*r+211*s)%65536 \
             value=(1+((17*r+29*j+31*s)%997))/1024 rounds={rounds} \
             construction_outside_timing=true serialization_outside_timing=true \
             parity_outside_timing=true two_arm_order=alternating \
             null_design=four-call-forward-reverse-geometric-symmetrization \
        ...
        let (mut scipy, identity, scipy_engine_sha256) =
            initialize_live_vstack(&script, &blocks, &input_sha256, &current_output_sha256)?;
        println!("scipy_arm: {identity}");
        println!("scipy_engine_sha256={scipy_engine_sha256}");
        println!(
            "input_sha256={input_sha256} frankenscipy_input_sha256={input_sha256} \
        ...
        println!(
            "agreement: current_output_sha256={current_output_sha256} \
             scipy_output_sha256={current_output_sha256} output_digest_match=true \
             shape={VSTACK_RESULT_ROWS}x{VSTACK_COLS} nnz={VSTACK_RESULT_NNZ} exact=true"
        );

        let quiescence_pre = sample_registered_lil_quiescence("pre")?;
        let mut current_repetitions = 1usize;
        let current_calibration_seconds = loop {
            let elapsed = time_vstack_csr(&block_refs, current_repetitions);
            if elapsed >= VSTACK_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            current_repetitions = current_repetitions
                .checked_mul(2)
                .ok_or_else(|| "current vstack calibration overflowed".to_string())?;
        };
        let mut live_repetitions = 1usize;
        let live_calibration_seconds = loop {
            let elapsed = scipy.solve(live_repetitions, VSTACK_RESULT_NNZ)?;
            if elapsed >= VSTACK_MIN_SAMPLE_SECONDS {
                break elapsed;
            }
            live_repetitions = live_repetitions
                .checked_mul(2)
                .ok_or_else(|| "live vstack calibration overflowed".to_string())?;
        };
        let duration_pass = current_calibration_seconds >= VSTACK_MIN_SAMPLE_SECONDS
            && live_calibration_seconds >= VSTACK_MIN_SAMPLE_SECONDS;
        println!(
            "calibration: current_repetitions={current_repetitions} \
             live_repetitions={live_repetitions} min_sample_ms=50 \
             current_seconds={current_calibration_seconds:.9} \
        ...
        for _ in 0..2 {
            let _ = time_vstack_csr(&block_refs, current_repetitions);
            let _ = scipy.solve(live_repetitions, VSTACK_RESULT_NNZ)?;
        }
        let quiescence_measurement = sample_registered_lil_quiescence("measurement")?;
        ...
        for round in 0..rounds {
            let (current_batch, live_batch) = if round.is_multiple_of(2) {
                (
                    time_vstack_csr(&block_refs, current_repetitions),
                    scipy.solve(live_repetitions, VSTACK_RESULT_NNZ)?,
                )
            } else {
                let live = scipy.solve(live_repetitions, VSTACK_RESULT_NNZ)?;
                let current = time_vstack_csr(&block_refs, current_repetitions);
                (current, live)
            };
            let current_null =
                four_call_geometric_null(|| Ok(time_vstack_csr(&block_refs, current_repetitions)))?;
            let live_null =
                four_call_geometric_null(|| scipy.solve(live_repetitions, VSTACK_RESULT_NNZ))?;
            let current_seconds = current_batch / current_repetitions as f64;
            let live_seconds = live_batch / live_repetitions as f64;
            current_times.push(current_seconds);
            live_times.push(live_seconds);
...
fn main() {
    ...
    if mode == "vstack-csr-current-profile" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            ...
            let repetitions = args
                ...
                .unwrap_or(1);
            if let Err(error) =
                expm_bench::run_vstack_csr_current_profile(rows_per_block, repetitions)
            {
        ...
        {
            eprintln!("canonical CSR vstack profiling requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
    if mode == "vstack-csr-parity" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let rows_per_block = args
                .get(2)
                .and_then(|value| value.parse().ok())
                .unwrap_or(32_768);
            if let Err(error) = expm_bench::run_vstack_csr_parity(rows_per_block, args.get(3)) {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
            return;
        ...
        #[cfg(not(feature = "sparse-incumbent-bench"))]
        {
            eprintln!("canonical CSR vstack parity requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
    if mode == "vstack-csr-vs-scipy" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let rows_per_block = args
                .get(2)
            ...
            let rounds = args
                ...
                .unwrap_or(24);
            if let Err(error) =
                expm_bench::run_vstack_csr_vs_scipy(rows_per_block, rounds, args.get(4))
            {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
        ...
        #[cfg(not(feature = "sparse-incumbent-bench"))]
        {
            eprintln!("canonical CSR vstack comparison requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
    ...
    if mode == "lil-skew-to-csr-current-profile" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            ...
            let repetitions = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(1);
            if let Err(error) = expm_bench::run_lil_skew_to_csr_current_profile(rows, repetitions) {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
            return;
        ...
        #[cfg(not(feature = "sparse-incumbent-bench"))]
        {
            eprintln!("skewed LIL-to-CSR profiling requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
    if mode == "lil-skew-to-csr-vs-scipy" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let rows = args
                .get(2)
            ...
            let rounds = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(24);
            if let Err(error) = expm_bench::run_lil_skew_to_csr_vs_scipy(rows, rounds, args.get(4))
            {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
    ...
    if mode == "bsr-to-csr-current-profile" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            ...
            let repetitions = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(1);
            if let Err(error) = expm_bench::run_bsr_to_csr_current_profile(n, repetitions) {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
            return;
        ...
        #[cfg(not(feature = "sparse-incumbent-bench"))]
        {
            eprintln!("BSR-to-CSR profiling requires --features sparse-incumbent-bench");
            std::process::exit(2);
        }
    }
    if mode == "bsr-to-csr-vs-scipy" {
        #[cfg(feature = "sparse-incumbent-bench")]
        {
            let n = args
                .get(2)
            ...
            let rounds = args
                ...
                .and_then(|value| value.parse().ok())
                .unwrap_or(24);
            if let Err(error) = expm_bench::run_bsr_to_csr_vs_scipy(n, rounds, args.get(4)) {
                eprintln!("fatal: {error}");
                std::process::exit(2);
            }
            return;
