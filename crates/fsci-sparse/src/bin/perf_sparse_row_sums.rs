//! Median-null-gated A/B for `sparse::sparse_row_sums`: ORIG serial per-row Σ vs row-chunked
//! parallel (each out[i] folds its own disjoint data slice, order preserved -> BYTE-IDENTICAL).
//! Toggled by the shared `SPARSE_ROW_MINMAX_FORCE_SERIAL`. Args: rows [nnz_per_row] [iters].
use fsci_sparse::{CsrMatrix, SPARSE_ROW_MINMAX_FORCE_SERIAL, Shape2D, sparse_row_sums};
...
fn main() {
    ...
    println!(
        "{} sparse_row_sums serial {ob:.2}ms (cv {:.1}%) parallel {fb:.2}ms (cv {:.1}%) | \
