...
def lil_to_csr_fixture(
    ...
    for row in range(n):
        entries = [
            ((1021 * slot + 73 * row) % n, slot)
            for slot in range(entries_per_row)
        ]
        entries.sort(key=lambda entry: entry[0])
        for column, slot in entries:
            indices[offset] = column
            data[offset] = (1 + ((17 * row + 19 * slot) % 509)) / 512.0
            offset += 1
    matrix = lil_from_flat_parts(n, n, data, indices, indptr)
...
def profile_sparse_lil_to_csr(repetitions: int, n: int) -> int:
    if repetitions < 1 or n != 65536:
        print("LIL_TO_CSR_SCIPY_FATAL invalid-controls", flush=True)
        return 2
    try:
        engine_path, engine_sha256, genuine = sparse_lil_to_csr_identity()
        matrix, data, indices, indptr = lil_to_csr_fixture(n)
    except RuntimeError as error:
        print(f"LIL_TO_CSR_SCIPY_FATAL {error}", flush=True)
        return 2
    print(
        f"LIL_TO_CSR_SCIPY_READY scipy={scipy.__version__} numpy={np.__version__} "
        f"solver_mod={sp.lil_matrix.tocsr.__module__} "
        f"scipy_engine_file={engine_path} scipy_engine_sha256={engine_sha256} "
        f"genuine={genuine}",
        flush=True,
    )
    if not genuine:
        print("LIL_TO_CSR_SCIPY_FATAL not-genuine-scipy", flush=True)
        return 2
    ...
    assert result is not None
    print(
        f"LIL_TO_CSR_SCIPY_PROFILE n={n} entries_per_row=32 nnz={matrix.nnz} "
        f"repetitions={repetitions} elapsed_seconds={elapsed:.9f} "
        ...
        f"actual_observed_worker_threads={maximum_threads} "
        f"input_sha256={lil_input_sha256_parts(n, n, data, indices, indptr)}",
        flush=True,
    )
    return 0


def live_sparse_lil_to_csr() -> int:
    try:
        engine_path, engine_sha256, genuine = sparse_lil_to_csr_identity()
    except RuntimeError as error:
        print(f"FATAL {error}", flush=True)
    ...
    fsci_loaded = any(name.startswith(("fsci", "franken")) for name in sys.modules)
    print(
        f"READY scipy={scipy.__version__} numpy={np.__version__} method=lil_to_csr "
        f"solver_mod={sp.lil_matrix.tocsr.__module__} "
        f"scipy_file={Path(scipy.__file__).resolve()} "
        f"scipy_engine_file={engine_path} scipy_engine_sha256={engine_sha256} "
        f"python={Path(sys.executable).resolve()} "
    ...
    for raw_line in sys.stdin:
        ...
        if parts[0] == "INIT_LIL":
            ...
            print(
                f"CASE method=lil_to_csr rows={rows} cols={cols} nnz={matrix.nnz} "
                f"entries_per_row={nnz // rows if rows else 0} "
...
def main() -> int:
    if len(sys.argv) == 4 and sys.argv[1] == "--profile-lil-to-csr":
        return profile_sparse_lil_to_csr(int(sys.argv[2]), int(sys.argv[3]))
    if len(sys.argv) == 2 and sys.argv[1] == "--live-lil-to-csr":
        return live_sparse_lil_to_csr()
