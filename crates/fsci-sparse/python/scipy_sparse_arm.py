...
def coo_add_operand(n: int, side: int) -> sp.coo_matrix:
    entries_per_row = 48
    ...
    for row in range(n):
        entries = [
            (
                (257 * slot + 31 * row + 6168 * side) % n,
                (1 + ((13 * row + 17 * slot + 23 * side) % 251)) / 256.0,
            )
    ...
    if not coo_is_strictly_lexicographic(matrix.row, matrix.col):
        raise RuntimeError("generated COO-add operand is not sorted and unique")
    return matrix
...
def sparse_coo_add_identity() -> tuple[Path, str, bool]:
    engine = sp.coo_matrix._add_sparse
    engine_path_text = inspect.getsourcefile(engine)
    if engine_path_text is None:
        raise RuntimeError("COO-add engine source is unavailable")
    engine_path = Path(engine_path_text).resolve()
    scipy_path = Path(scipy.__file__).resolve()
    installed = any(
        part in {"site-packages", "dist-packages"} for part in scipy_path.parts
...
def profile_sparse_coo_add(repetitions: int, n: int) -> int:
    if repetitions < 1 or n != 16384:
        print("COO_ADD_SCIPY_FATAL invalid-controls", flush=True)
        return 2
    try:
        engine_path, engine_sha256, genuine = sparse_coo_add_identity()
        lhs = coo_add_operand(n, 0)
        rhs = coo_add_operand(n, 1)
    except RuntimeError as error:
        print(f"COO_ADD_SCIPY_FATAL {error}", flush=True)
        return 2
    print(
        f"COO_ADD_SCIPY_READY scipy={scipy.__version__} numpy={np.__version__} "
        f"solver_mod={sp.coo_matrix._add_sparse.__module__} "
        f"scipy_engine_file={engine_path} scipy_engine_sha256={engine_sha256} "
        f"genuine={genuine}",
        flush=True,
    )
    if not genuine:
        print("COO_ADD_SCIPY_FATAL not-genuine-scipy", flush=True)
        return 2
    input_sha256 = coo_pair_sha256(lhs, rhs)
    warm = lhs + rhs
    if not isinstance(warm, sp.csr_matrix) or warm.nnz != 1179648:
        print("COO_ADD_SCIPY_FATAL canonical-csr-result-required", flush=True)
        return 2
    checksum = 0.0
    maximum_threads = observed_threads()
    started = time.perf_counter()
    ...
    maximum_threads = max(maximum_threads, observed_threads())
    print(
        f"COO_ADD_SCIPY_PROFILE n={n} lhs_nnz={lhs.nnz} rhs_nnz={rhs.nnz} "
        f"repetitions={repetitions} elapsed_seconds={elapsed:.9f} "
...
def live_sparse_coo_add() -> int:
    ...
    fsci_loaded = any(name.startswith(("fsci", "franken")) for name in sys.modules)
    print(
        f"READY scipy={scipy.__version__} numpy={np.__version__} method=coo_add "
        f"solver_mod={sp.coo_matrix._add_sparse.__module__} "
        f"scipy_file={Path(scipy.__file__).resolve()} "
        f"scipy_engine_file={engine_path} scipy_engine_sha256={engine_sha256} "
        f"python={Path(sys.executable).resolve()} "
    ...
    for raw_line in sys.stdin:
        ...
        if parts[0] == "INIT_COO_ADD":
            ...
            if not isinstance(warm, sp.csr_matrix):
                ...
                return 2
            print(
                f"CASE method=coo_add rows={rows_count} cols={cols_count} "
...
def main() -> int:
    if len(sys.argv) == 4 and sys.argv[1] == "--profile-coo-add":
        return profile_sparse_coo_add(int(sys.argv[2]), int(sys.argv[3]))
    if len(sys.argv) == 2 and sys.argv[1] == "--live-coo-add":
        return live_sparse_coo_add()
