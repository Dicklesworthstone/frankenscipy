...
def csr_add_operand(n: int, side: int) -> sp.csr_matrix:
    entries_per_row = 64
    ...
    for row in range(n):
        entries = [
            (
                (257 * slot + 31 * row + 8224 * side) % n,
                (1 + ((11 * row + 7 * slot + 19 * side) % 61)) / 128.0,
            )
    ...
    matrix = sp.csr_matrix((data, indices, indptr), shape=(n, n), copy=False)
    if not matrix.has_sorted_indices or not matrix.has_canonical_format:
        raise RuntimeError("generated CSR-add operand is not canonical")
    return matrix


def csr_pair_sha256(lhs: sp.csr_matrix, rhs: sp.csr_matrix) -> str:
    digest = hashlib.sha256()
    digest.update(struct.pack("<Q", lhs.shape[0]))
    digest.update(struct.pack("<Q", lhs.shape[1]))
    for matrix in (lhs, rhs):
        digest.update(struct.pack("<Q", int(matrix.nnz)))
        digest.update(np.asarray(matrix.data, dtype="<f8").tobytes(order="C"))
        digest.update(np.asarray(matrix.indices, dtype="<u8").tobytes(order="C"))
...
def sparse_csr_add_identity() -> tuple[Path, str, bool]:
    engine = sp.csr_matrix._binopt
    engine_path_text = inspect.getsourcefile(engine)
    if engine_path_text is None:
        raise RuntimeError("CSR-add engine source is unavailable")
    engine_path = Path(engine_path_text).resolve()
    scipy_path = Path(scipy.__file__).resolve()
    installed = any(
        part in {"site-packages", "dist-packages"} for part in scipy_path.parts
...
def profile_sparse_csr_add(repetitions: int, n: int) -> int:
    if repetitions < 1 or n != 8192:
        print("CSR_ADD_SCIPY_FATAL invalid-controls", flush=True)
        return 2
    try:
        engine_path, engine_sha256, genuine = sparse_csr_add_identity()
        lhs = csr_add_operand(n, 0)
        rhs = csr_add_operand(n, 1)
    except RuntimeError as error:
        print(f"CSR_ADD_SCIPY_FATAL {error}", flush=True)
        return 2
    print(
        f"CSR_ADD_SCIPY_READY scipy={scipy.__version__} numpy={np.__version__} "
        f"solver_mod={sp.csr_matrix._binopt.__module__} "
        f"scipy_engine_file={engine_path} scipy_engine_sha256={engine_sha256} "
        f"genuine={genuine}",
        flush=True,
    )
    if not genuine:
        print("CSR_ADD_SCIPY_FATAL not-genuine-scipy", flush=True)
        return 2
    warm = lhs + rhs
    if not isinstance(warm, sp.csr_matrix) or warm.nnz != n * 96:
        print("CSR_ADD_SCIPY_FATAL csr-result-contract", flush=True)
        return 2
    ...
    checksum = float(result.data.sum()) + float(result.nnz)
    print(
        f"CSR_ADD_SCIPY_PROFILE n={n} entries_per_row=64 lhs_nnz={lhs.nnz} "
        f"rhs_nnz={rhs.nnz} repetitions={repetitions} elapsed_seconds={elapsed:.9f} "
        f"checksum={checksum:.17e} result_format={result.format} "
        f"result_nnz={result.nnz} sorted={result.has_sorted_indices} "
        f"canonical={result.has_canonical_format} "
        f"actual_observed_worker_threads={maximum_threads} "
        f"input_sha256={csr_pair_sha256(lhs, rhs)}",
        flush=True,
    )
    return 0


def live_sparse_csr_add() -> int:
    try:
        engine_path, engine_sha256, genuine = sparse_csr_add_identity()
    except RuntimeError as error:
        print(f"FATAL {error}", flush=True)
    ...
    fsci_loaded = any(name.startswith(("fsci", "franken")) for name in sys.modules)
    print(
        f"READY scipy={scipy.__version__} numpy={np.__version__} method=csr_add "
        f"solver_mod={sp.csr_matrix._binopt.__module__} "
        f"scipy_file={Path(scipy.__file__).resolve()} "
        f"scipy_engine_file={engine_path} scipy_engine_sha256={engine_sha256} "
        f"python={Path(sys.executable).resolve()} "
    ...
    lhs: sp.csr_matrix | None = None
    rhs: sp.csr_matrix | None = None
    input_sha256: str | None = None
    for raw_line in sys.stdin:
        line = raw_line.strip()
        parts = line.split()
        ...
        if parts[0] == "QUIT":
            break
        if parts[0] == "INIT_CSR_ADD":
            if len(parts) != 5:
                print(f"FATAL bad-init {line}", flush=True)
                return 2
            rows, cols = int(parts[1]), int(parts[2])
            lhs_nnz, rhs_nnz = int(parts[3]), int(parts[4])
            try:
                lhs_indptr = parse_vector(
                    sys.stdin.readline(), "LHS_INDPTR", rows + 1, np.int64
                )
                lhs_indices = parse_vector(
                    sys.stdin.readline(), "LHS_INDICES", lhs_nnz, np.int64
                )
                ...
                rhs_indptr = parse_vector(
                    sys.stdin.readline(), "RHS_INDPTR", rows + 1, np.int64
                )
                rhs_indices = parse_vector(
                    sys.stdin.readline(), "RHS_INDICES", rhs_nnz, np.int64
                )
                ...
                return 2
            lhs = sp.csr_matrix(
                (lhs_data, lhs_indices, lhs_indptr), shape=(rows, cols), copy=False
            )
            rhs = sp.csr_matrix(
                (rhs_data, rhs_indices, rhs_indptr), shape=(rows, cols), copy=False
            )
            input_sha256 = csr_pair_sha256(lhs, rhs)
            finite = bool(np.isfinite(lhs_data).all() and np.isfinite(rhs_data).all())
            warm = lhs + rhs
            if not isinstance(warm, sp.csr_matrix):
                print("FATAL csr-result-required", flush=True)
                return 2
            print(
                f"CASE method=csr_add rows={rows} cols={cols} lhs_nnz={lhs.nnz} "
                f"rhs_nnz={rhs.nnz} lhs_sorted={lhs.has_sorted_indices} "
                f"rhs_sorted={rhs.has_sorted_indices} "
                f"lhs_canonical={lhs.has_canonical_format} "
                f"rhs_canonical={rhs.has_canonical_format} finite={finite} "
                f"result_nnz={warm.nnz}",
                flush=True,
            )
            continue
        if parts[0] == "INPUT_SHA256":
...
def main() -> int:
    if len(sys.argv) == 4 and sys.argv[1] == "--profile-csr-add":
        return profile_sparse_csr_add(int(sys.argv[2]), int(sys.argv[3]))
    if len(sys.argv) == 2 and sys.argv[1] == "--live-csr-add":
        return live_sparse_csr_add()
