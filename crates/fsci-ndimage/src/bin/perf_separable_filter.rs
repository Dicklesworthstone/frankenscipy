...
#[derive(Clone, Copy)]
enum Op { Gaussian, Uniform }

impl Op {
    fn name(self) -> &'static str { match self { Self::Gaussian => "gaussian", Self::Uniform => "uniform" } }
...
struct Scipy { child: Child, stdin: ChildStdin, stdout: BufReader<ChildStdout> }

impl Scipy {
    fn start(input: &NdArray, op: Op) -> Self {
        let shape = input.shape.iter().map(usize::to_string).collect::<Vec<_>>().join(",");
        let mut child = Command::new("python3").arg("-u").arg("-c").arg(PYTHON)
            .env("PYTHONPATH", SCIPY_SITE_PACKAGES).env("FSCI_NDIMAGE_SHAPE", shape)
            .env("FSCI_NDIMAGE_OP", op.name()).stdin(Stdio::piped()).stdout(Stdio::piped())
            .stderr(Stdio::inherit()).spawn().expect("start SciPy");
        let mut stdin = child.stdin.take().expect("SciPy stdin");
        for &value in &input.data { stdin.write_all(&value.to_le_bytes()).expect("send fixture"); }
        stdin.flush().expect("flush fixture");
        let stdout = BufReader::new(child.stdout.take().expect("SciPy stdout"));
        let mut this = Self { child, stdin, stdout };
        let ready = this.line();
        assert!(ready.starts_with("READY ") && ready.contains("genuine=True"), "bad incumbent: {ready}");
        println!("op={} {ready}", op.name());
        this
    }
    fn line(&mut self) -> String {
        let mut line = String::new(); self.stdout.read_line(&mut line).expect("SciPy response"); line.trim().to_owned()
    }
    fn time(&mut self, reps: usize, min_of: usize) -> f64 {
        writeln!(self.stdin, "TIME {reps} {min_of}").expect("TIME request"); self.stdin.flush().expect("TIME flush");
        self.line().strip_prefix("TIME ").expect("TIME response").parse().expect("finite time")
    }
    fn check(&mut self, ours: &NdArray) -> String {
        writeln!(self.stdin, "CHECK").expect("CHECK request");
        for &value in &ours.data { self.stdin.write_all(&value.to_le_bytes()).expect("send result"); }
        self.stdin.flush().expect("CHECK flush");
        let line = self.line(); assert!(line.starts_with("CHECK "), "CHECK response: {line}"); line
    }
}
...
fn fixture(shape: &[usize]) -> NdArray {
    ...
    let data = (0..shape.iter().product()).map(|index| {
...
fn time_fsci(op: Op, input: &NdArray, reps: usize, min_of: usize) -> f64 {
    ...
    for _ in 0..min_of {
        ...
        for _ in 0..reps { let output = op.run(input); black_box(output.data[0]); }
...
fn elf_sha256() -> String {
    let output = Command::new("sha256sum").arg(std::env::current_exe().expect("executable")).output().expect("sha256sum");
    assert!(output.status.success(), "sha256sum failed");
    String::from_utf8(output.stdout).expect("sha256 output").split_whitespace().next().expect("digest").to_owned()
}

fn main() {
    let cases: &[(&str, &[usize])] = &[("2d-4096", &[4096, 4096]), ("3d-256", &[256, 256, 256])];
    let reps = 1; let min_of = 2;
    println!("# live scipy.ndimage whole-job separable filters");
    println!("elf_sha256={}", elf_sha256());
    println!("host={} affinity={} reps={reps} min_of={min_of}", std::fs::read_to_string("/proc/sys/kernel/hostname").unwrap_or_default().trim(), std::thread::available_parallelism().map_or(0, std::num::NonZeroUsize::get));
    ...
    for &(case, shape) in cases {
        ...
        for op in [Op::Gaussian, Op::Uniform] {
            ...
            let fsci = f_a.min(f_b); let incumbent = s_a.min(s_b);
            println!("case={case} op={} fsci={:.3}ms scipy={:.3}ms scipy/fsci={:.3}x null_fsci={:.3} null_scipy={:.3} {parity}", op.name(), fsci * 1e3, incumbent * 1e3, incumbent / fsci, f_a.max(f_b) / f_a.min(f_b), s_a.max(s_b) / s_a.min(s_b));
        }
    }
}
