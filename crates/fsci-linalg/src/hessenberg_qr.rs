...
pub(crate) fn real_schur_francis(
    ...
    loop {
        ...
        let mut bulge_z = h10 * h.get(ilo + 2, ilo + 1);

        for k in ilo..ihi - 1 {
            // A 3-element Householder reflector zeroing y and bulge_z.
            let norm_sq = x * x + y * y + bulge_z * bulge_z;
            if norm_sq != 0.0 {
                let norm = norm_sq.sqrt();
                let beta = if x >= 0.0 { -norm } else { norm };
                let v0 = x - beta;
                let vnorm_sq = v0 * v0 + y * y + bulge_z * bulge_z;
                if vnorm_sq != 0.0 {
                    let two_over = 2.0 / vnorm_sq;
                    let v = [v0, y, bulge_z];
                    let rows = [k, k + 1, k + 2];
            ...
            if k + 1 < ihi {
                x = h.get(k + 1, k);
                y = h.get(k + 2, k);
                bulge_z = if k + 3 <= ihi { h.get(k + 3, k) } else { 0.0 };
            }
        }
