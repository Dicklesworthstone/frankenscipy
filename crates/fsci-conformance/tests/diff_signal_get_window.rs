...
"#;
    ...
    };
    {
        ...
        if let Err(err) = stdin.write_all(query_json.as_bytes()) {
            ...
            eprintln!("skipping get_window oracle: stdin write failed ({err})\n{stderr}");
            return None;
        }
    }
    let output = child
        .wait_with_output()
        .expect("wait for get_window oracle");
...
fn diff_signal_get_window() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_get_window", &["get_window"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = get_window(&case.spec, case.nx).ok();
        ...
        ledger.compared("get_window", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
