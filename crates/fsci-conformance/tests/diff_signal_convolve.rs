...
fn diff_signal_convolve() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_convolve", &["convolve"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = convolve(&case.a, &case.b, parse_mode(&case.mode)).ok();
        ...
        ledger.compared("convolve", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
