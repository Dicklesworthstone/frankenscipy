...
const ARMS: [&str; 2] = ["negentropy", "bce"];
...
fn diff_special_negentropy_bce() {
    ...
    let mut ledger = CompareLedger::new("diff_special_negentropy_bce", &ARMS);
    ...
    for case in &query.points {
        ...
        let Some((expected, actual)) = ledger.pair(
            ...
            &case.case_id,
            scipy,
            fsci_eval(&case.op, case.p, case.q),
        ) else {
            continue;
        };
    ...
    // Arms have different case sets (negentropy has the fewest); each must compare all of its own.
