...
fn diff_optimize_find_minimum() {
    ...
    let mut ledger = CompareLedger::new("diff_optimize_find_minimum", &["find_minimum"]);
    ...
    for (case, arm) in query.points.iter().zip(&oracle.points) {
        ...
        let Some((
            (sampled, x_bits, f_x_bits, bracket, f_bracket, nit, nfev, status, success),
            result,
        )) = ledger.both("find_minimum", &case.case_id, scipy, fsci)
        else {
            continue;
        };
        ...
        ledger.compared("find_minimum", &case.case_id, true);
        compared += 1;
