...
fn diff_sparse_iterative_solvers() {
    ...
    // Solvers run on different system sets (minres/gmres also get the indefinite one);
    // each must compare every case it is designed for, so the minimum bounds all arms.
