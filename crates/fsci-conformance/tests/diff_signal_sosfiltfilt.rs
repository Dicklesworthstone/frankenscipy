...
fn diff_signal_sosfiltfilt() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_sosfiltfilt", &["sosfiltfilt"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let sos: Vec<SosSection> = case.sos.to_vec();
        let fsci_v = sosfiltfilt(&sos, &case.x).ok();
        ...
        ledger.compared("sosfiltfilt", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
