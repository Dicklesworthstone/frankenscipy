...
fn diff_signal_lfilter() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_lfilter", &["lfilter"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = lfilter(&case.b, &case.a, &case.x, None).ok();
        ...
        ledger.compared("lfilter", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
