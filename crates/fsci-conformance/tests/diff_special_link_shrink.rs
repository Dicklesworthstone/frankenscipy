...
/// One ledger arm per helper compared.
const ARMS: [&str; 9] = [
    "cloglog",
    "loglog",
    "cauchit",
...
fn diff_special_link_shrink() {
    ...
    let mut ledger = CompareLedger::new("diff_special_link_shrink", &ARMS);
    ...
    for case in &query.points {
        ...
        let Some((expected, actual)) = ledger.pair(
            ...
            &case.case_id,
            scipy,
            fsci_eval(&case.op, case.x, case.lambda),
        ) else {
            continue;
        };
    ...
    // Arms have different case sets (the inverse links have the fewest); each must compare all
    // of its own.
