...
fn diff_stats_chi2_contingency_lambda() {
    ...
    let mut ledger = CompareLedger::new(
        "diff_stats_chi2_contingency_lambda",
    ...
    for case in &query.points {
        ...
        let Some((s_dof, rust_dof)) =
            ledger.both("dof", &case.case_id, scipy_arm.dof, Some(result.dof))
        else {
            continue;
        };
        let abs_diff = (rust_dof as i64 - s_dof).unsigned_abs() as f64;
        max_overall = max_overall.max(abs_diff);
        ledger.compared("dof", &case.case_id, abs_diff == 0.0);
        diffs.push(CaseDiff {
            case_id: case.case_id.clone(),
            arm: "dof".into(),
            abs_diff,
