...
fn diff_stats_kstat() {
    ...
    let mut ledger = CompareLedger::new("diff_stats_kstat", &["kstat", "kstatvar"]);
    ...
    // kstat runs n = 1..=4 per dataset; kstatvar only n = 1, 2 (the smaller arm).
    ledger.finish(query.points.iter().filter(|c| c.func == "kstatvar").count());
}
