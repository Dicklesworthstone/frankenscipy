...
fn diff_signal_peak_widths() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_peak_widths", &["peak_widths"]);
    ...
    for case in &query.points {
        ...
        // The packed lengths agree; the four-way split must agree with SciPy's n too.
        let abs_d = if scipy_arm.n.is_some_and(|n| fsci_v.len() == 4 * n) {
            fsci_v
                .iter()
                .zip(scipy_v.iter())
                .map(|(a, b)| (a - b).abs())
        ...
        ledger.compared("peak_widths", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
