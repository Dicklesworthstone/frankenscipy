...
fn diff_special_powm1() {
    ...
    let mut ledger = CompareLedger::new("diff_special_powm1", &["powm1"]);
    ...
    for case in &query.points {
        ...
        let Some((expected, actual)) = ledger.both("powm1", &case.case_id, scipy, fsci) else {
            continue;
        };
        let diff = compare(case, actual, expected);
        max_overall = max_overall.max(diff.abs_diff);
        ledger.compared("powm1", &case.case_id, diff.pass);
        diffs.push(diff);
    }
