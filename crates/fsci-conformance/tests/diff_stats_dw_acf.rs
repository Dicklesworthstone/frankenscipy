...
const ARMS: [&str; 2] = ["durbin_watson", "acf"];
...
fn generate_query() -> OracleQuery {
    ...
    let acf_fixtures: Vec<(&str, Vec<f64>, u64)> = vec![
        ...
        (
            "noisy_lag3",
            vec![1.0, 1.5, 0.8, 1.7, 0.9, 1.4, 1.0, 1.6, 1.1, 1.5, 0.7, 1.4],
            3,
        ),
        (
            "alternating_lag4",
...
fn diff_stats_dw_acf() {
    ...
    let mut ledger = CompareLedger::new("diff_stats_dw_acf", &ARMS);
    ...
    // acf has 3 fixtures to durbin_watson's 4; each arm must compare all of its own.
