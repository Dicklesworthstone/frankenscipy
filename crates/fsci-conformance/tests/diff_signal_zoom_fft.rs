...
fn diff_signal_zoom_fft() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_zoom_fft", &["zoom_fft"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = zoom_fft(&case.x, (case.f1, case.f2), case.m)
            .ok()
            .map(|complex_out| {
                let mut packed = Vec::with_capacity(complex_out.len() * 2);
        ...
        ledger.compared("zoom_fft", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
