...
/// One ledger arm per spherical Bessel function.
const ARMS: [&str; 4] = ["jn", "yn", "in_", "kn"];
...
"#;
    ...
    };
    {
        ...
        if let Err(err) = stdin.write_all(query_json.as_bytes()) {
            ...
            eprintln!("skipping spherical-bessel oracle: stdin write failed ({err})\n{stderr}");
            return None;
        }
    }
    let output = child
...
fn diff_special_spherical_bessel() {
    ...
    let mut ledger = CompareLedger::new("diff_special_spherical_bessel", &ARMS);
    ...
    for case in &query.points {
        ...
        let Some((scipy_v, rust_v)) = ledger.pair(
            ...
            &case.case_id,
            oracle.value,
            fsci_eval(&case.func, case.n, case.z),
        ) else {
            continue;
        };
    ...
    // Each function must compare every (n, z) pair.
