...
"#;
    ...
    let output = child
        .wait_with_output()
        .expect("wait for erf_complex oracle");
...
fn diff_special_erf_complex() {
    ...
    let mut ledger = CompareLedger::new("diff_special_erf_complex", &ARMS);
    ...
    for case in &query.points {
        ...
        let Some(((ere, eim), (re, im))) = ledger.both(
            ...
            &case.case_id,
            scipy,
            fsci_eval(&case.op, case.z_re, case.z_im),
        ) else {
            continue;
        };
