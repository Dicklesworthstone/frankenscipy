...
"#;
    ...
    };
    {
        let stdin = child
            .stdin
            .as_mut()
            .expect("open halflogistic oracle stdin");
    ...
    let output = child
        .wait_with_output()
        .expect("wait for halflogistic oracle");
...
fn diff_stats_halflogistic() {
    ...
    let mut ledger = CompareLedger::new("diff_stats_halflogistic", &["pdf", "cdf", "ppf"]);
    ...
    // pdf/cdf compare every x point; ppf has its own (smaller) q-grid.
