...
fn diff_signal_butter() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_butter", &["butter"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let coeffs = butter(case.order, &[case.wn], FilterType::Lowpass).ok();
        ...
        ledger.compared("butter", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
