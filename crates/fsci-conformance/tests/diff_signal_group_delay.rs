...
fn diff_signal_group_delay() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_group_delay", &["group_delay"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = group_delay(&case.b, &case.a, Some(case.n))
            .ok()
            .map(|(w, gd)| {
                let mut v = w;
                v.extend(gd);
        ...
        ledger.compared("group_delay", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
