...
fn diff_signal_lfilter_zi() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_lfilter_zi", &["lfilter_zi"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = lfilter_zi(&case.b, &case.a).ok();
        ...
        ledger.compared("lfilter_zi", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
