...
const ARMS: [&str; 4] = ["jvp", "yvp", "ivp", "kvp"];
...
"#;
    ...
    };
    {
        ...
        if let Err(err) = stdin.write_all(query_json.as_bytes()) {
            ...
            eprintln!("skipping bessel-derivs oracle: stdin write failed ({err})\n{stderr}");
            return None;
        }
    }
    let output = child
...
fn diff_special_bessel_derivs() {
    ...
    let mut ledger = CompareLedger::new("diff_special_bessel_derivs", &ARMS);
    ...
    for case in &query.points {
        ...
        let Some((scipy_v, rust_v)) = ledger.pair(
            ...
            &case.case_id,
            oracle.value,
            fsci_eval(&case.func, case.v, case.z),
        ) else {
            continue;
        };
    ...
    // Arms have different case sets (yvp has the fewest); each must compare all of its own.
