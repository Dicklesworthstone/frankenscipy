...
fn diff_signal_resample_poly() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_resample_poly", &["resample_poly"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = resample_poly(&case.x, case.up, case.down).ok();
        ...
        ledger.compared("resample_poly", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
