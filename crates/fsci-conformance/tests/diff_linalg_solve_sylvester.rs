...
fn diff_linalg_solve_sylvester() {
    ...
    let mut ledger = CompareLedger::new("diff_linalg_solve_sylvester", &["solve_sylvester"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v: Option<Vec<f64>> =
            solve_sylvester(&case.a, &case.b, &case.q, DecompOptions::default())
                .ok()
        ...
        ledger.compared("solve_sylvester", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
