...
fn diff_ndimage_gaussian_filter() {
    ...
    let mut ledger = CompareLedger::new("diff_ndimage_gaussian_filter", &["gaussian_filter"]);
    ...
    for case in &query.points {
        ...
        let fsci_v = NdArray::new(case.input.clone(), case.input_shape.clone())
            .ok()
            .and_then(|input| {
                gaussian_filter(&input, case.sigma, parse_mode(&case.mode), case.cval).ok()
            })
            .map(|out| out.data);
        ...
        ledger.compared("gaussian_filter", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
