...
fn diff_integrate_cumulative_simpson() {
    ...
    let mut ledger =
        CompareLedger::new("diff_integrate_cumulative_simpson", &["cumulative_simpson"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = cumulative_simpson(&case.y, &case.x).ok();
        ...
        ledger.compared("cumulative_simpson", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
