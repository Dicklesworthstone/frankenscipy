...
fn diff_special_pdtrik() {
    ...
    let mut ledger = CompareLedger::new("diff_special_pdtrik", &["pdtrik"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = pdtrik(case.p, case.m);
        let Some((scipy_v, fsci_v)) =
            ledger.pair("pdtrik", &case.case_id, scipy_arm.value, Some(fsci_v))
        else {
            continue;
        };
        let abs_d = (fsci_v - scipy_v).abs();
        ledger.compared("pdtrik", &case.case_id, abs_d <= ABS_TOL);
