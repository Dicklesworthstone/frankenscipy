...
/// One ledger arm per output of `sici`.
const ARMS: [&str; 2] = ["Si", "Ci"];
...
fn diff_special_sici() {
    ...
    let mut ledger = CompareLedger::new("diff_special_sici", &ARMS);
    ...
    // Arms have different case sets (Ci covers x > 0 only); each must compare all of its own.
