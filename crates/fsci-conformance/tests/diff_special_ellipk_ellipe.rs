...
const ARMS: [&str; 3] = ["ellipk", "ellipe", "ellipkm1"];
...
fn generate_query() -> OracleQuery {
    ...
    let ms = [
        0.0_f64, 0.001, 0.01, 0.1, 0.25, 0.4, 0.5, 0.6, 0.75, 0.9, 0.99, 0.999, 0.999999,
...
"#;
    ...
    };
    {
        ...
        if let Err(err) = stdin.write_all(query_json.as_bytes()) {
            ...
            eprintln!("skipping ellipk/ellipe oracle: stdin write failed ({err})\n{stderr}");
            return None;
        }
    }
    let output = child
        .wait_with_output()
        .expect("wait for ellipk/ellipe oracle");
...
fn diff_special_ellipk_ellipe() {
    ...
    let mut ledger = CompareLedger::new("diff_special_ellipk_ellipe", &ARMS);
    ...
    // Arms have different case sets (ellipkm1 has the fewest); each must compare all of its own.
