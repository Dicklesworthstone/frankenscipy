...
fn diff_opt_bracket() {
    ...
    let mut ledger = CompareLedger::new("diff_opt_bracket", &["bracket"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = fsci_target(&case.func).map(|f| {
            let (xa, xb, xc, fa, fb, fc) = bracket(f, case.xa, case.xb);
            vec![xa, xb, xc, fa, fb, fc]
        });
        ...
        ledger.compared("bracket", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
