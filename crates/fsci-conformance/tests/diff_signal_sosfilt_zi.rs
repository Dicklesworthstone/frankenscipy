...
fn diff_signal_sosfilt_zi() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_sosfilt_zi", &["sosfilt_zi"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let sos: Vec<SosSection> = case.sos.to_vec();
        // Flatten zi as [z0_0, z0_1, z1_0, z1_1, ...]
        let fsci_v: Option<Vec<f64>> = sosfilt_zi(&sos).ok().map(|zi| {
            let mut flat = Vec::with_capacity(zi.len() * 2);
            for z in &zi {
                flat.push(z[0]);
                flat.push(z[1]);
            }
        ...
        ledger.compared("sosfilt_zi", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
