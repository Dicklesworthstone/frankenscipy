...
fn diff_integrate_gauss_kronrod_newton_cotes() {
    ...
    let mut ledger = CompareLedger::new("diff_integrate_gauss_kronrod_newton_cotes", &ARMS);
    ...
    for case in &query.points {
        ...
        let actual = match arm {
            ...
            "nc" => {
                let f = move |x: f64| f1d(&fname, x);
                newton_cotes_quad(&f, case.a, case.b, case.n_panels, case.order).ok()
            }
            "cpv" => {
    ...
    // Arms have different case sets (`cpv` has the fewest); each must compare all of its own.
