...
const ARMS: [&str; 7] = ["clip", "abs", "exp", "reshape", "flatten", "full", "ones"];
...
"#;
    ...
    let output = child
        .wait_with_output()
        .expect("wait for clip_abs_exp oracle");
...
fn fsci_output(case: &Case) -> Option<Vec<f64>> {
    let actual: Vec<f64> = match case.op.as_str() {
        ...
        "reshape" => {
            let arr = NdArray::new(case.data.clone(), case.shape.clone()).ok()?;
            let out = reshape(&arr, case.target_shape.clone()).ok()?;
            out.data
        }
        "flatten" => {
            let arr = NdArray::new(case.data.clone(), case.shape.clone()).ok()?;
        ...
        other => panic!("unknown clip/abs/exp op `{other}`"),
    };
    Some(actual)
}
...
fn diff_ndimage_clip_abs_exp_reshape() {
    ...
    let mut ledger = CompareLedger::new("diff_ndimage_clip_abs_exp_reshape", &ARMS);

    for case in &query.points {
        let arm = pmap.get(&case.case_id).expect("oracle row for every case");
        let actual = fsci_output(case);
    ...
    // Arms have different case sets (flatten/full/ones have one); each must compare all its own.
