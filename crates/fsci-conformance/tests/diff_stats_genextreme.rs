...
"#;
    ...
    {
        let stdin = child.stdin.as_mut().expect("open genextreme oracle stdin");
    ...
    let output = child
        .wait_with_output()
        .expect("wait for genextreme oracle");
...
fn diff_stats_genextreme() {
    ...
    let mut ledger = CompareLedger::new("diff_stats_genextreme", &["pdf", "cdf", "sf", "ppf"]);
