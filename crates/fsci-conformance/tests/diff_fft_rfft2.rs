...
fn diff_fft_rfft2() {
    ...
    let mut ledger = CompareLedger::new("diff_fft_rfft2", &["rfft2"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let opts = FftOptions::default();
        let fsci_v = rfft2(&case.input, (case.rows, case.cols), &opts)
            .ok()
            .map(|out| out.iter().flat_map(|c| [c.0, c.1]).collect::<Vec<f64>>());
        ...
        ledger.compared("rfft2", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
