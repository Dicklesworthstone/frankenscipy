#![forbid(unsafe_code)]
//! Live scipy.special.psi parity for the explicit fsci_special::psi API.
//!
...
fn ensure_output_dir() {
    fs::create_dir_all(output_dir()).expect("create psi diff dir");
}
...
fn fsci_eval(case: &PointCase) -> PointArm {
    ...
    let result = psi(&SpecialTensor::RealScalar(input), RuntimeMode::Strict);
...
fn generate_query() -> OracleQuery {
    let points = vec![
        ...
        finite_case("pos_8", 8.0),
        finite_case("pole_zero", 0.0),
        finite_case("pole_neg_2", -2.0),
...
for case in q["points"]:
    cid = case["case_id"]
    try:
        r = float(special.psi(input_value(case)))
        ...
        .spawn()
    {
        ...
        Err(e) => {
            assert!(
                ...
                "failed to spawn python3 for psi oracle: {e}"
            );
            eprintln!("skipping psi oracle: python3 not available ({e})");
            return None;
        }
    };
    {
        ...
        if let Err(err) = stdin.write_all(script.as_bytes()) {
            ...
            assert!(
                std::env::var(REQUIRE_SCIPY_ENV).is_err(),
                "psi oracle script write failed: {err}; stderr: {stderr}"
            );
            eprintln!("skipping psi oracle: script write failed ({err})\n{stderr}");
            return None;
        }
    }
    let output = child.wait_with_output().expect("wait for psi oracle");
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            std::env::var(REQUIRE_SCIPY_ENV).is_err(),
            "psi oracle failed: {stderr}"
        );
        eprintln!("skipping psi oracle: scipy not available\n{stderr}");
        return None;
    }
    let stdout = String::from_utf8_lossy(&output.stdout);
    Some(serde_json::from_str(&stdout).expect("parse psi oracle JSON"))
}
...
fn diff_special_psi() {
    ...
    for d in &diffs {
        if !d.pass {
            eprintln!("psi mismatch: {} abs_diff={}", d.case_id, d.abs_diff);
        }
    }

    assert!(
        all_pass,
        "psi conformance failed: {} cases, max_diff={}",
        diffs.len(),
        max_overall
    );
}
