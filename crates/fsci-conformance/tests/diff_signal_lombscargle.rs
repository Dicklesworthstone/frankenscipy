...
fn diff_signal_lombscargle() {
    ...
    let mut ledger = CompareLedger::new("diff_signal_lombscargle", &["lombscargle"]);

    for case in &query.points {
        let scipy_arm = pmap.get(&case.case_id).expect("validated oracle");
        let fsci_v = lombscargle(&case.x, &case.y, &case.freqs, case.normalize).ok();
        ...
        ledger.compared("lombscargle", &case.case_id, abs_d <= ABS_TOL);
        diffs.push(CaseDiff {
