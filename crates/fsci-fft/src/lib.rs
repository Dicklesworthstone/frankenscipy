...
pub use transforms::{
    ...
    SyncSharedAuditLedger, TransformTrace, WorkerPolicy, dct, dct_i, dct_iii, dct_iv, dctn, dst,
    dst_i, dst_ii, dst_iii, dst_iv, dstn, fft, fft_with_audit, fft2, fft2_with_audit, fftn,
    fftn_with_audit, fht, fhtoffset, fwht, hfft, hfft_with_audit, hfft2, hfft2_with_audit, hfftn,
    hfftn_with_audit, hilbert, idct, idct_i, idct_iii, idct_iv, idctn, idst, idstn, ifft,
...
mod tests {
    ...
    fn large_prime_rader_path_matches_naive_dft() {
        // Restored in frankenscipy-ct8n2: deleted by 0b00356ee, whose stated
        // subject was reordering re-exports and reformatting. Primes n where
        // largest_prime_factor(n-1) ∈ {7,11,13} take the Rader route (previously
        // only 5-smooth n-1 did). Verify each matches a naive DFT and
        // round-trips — the existing proptests only cover n ≤ 32, so this path
        // was otherwise untested. n=1409 (1408=2^7·11), 1373 (1372=2^2·7^3),
        // 1093 (1092=2^2·3·7·13).
