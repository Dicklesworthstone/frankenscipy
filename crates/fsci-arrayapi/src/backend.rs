...
fn basic_unit_slice_bounds(
    spec: &SliceSpec,
    len: usize,
    mode: ExecutionMode,
) -> ArrayApiResult<(usize, usize)> {
    debug_assert_eq!(spec.step, 1);
    let len_i = len as isize;
    if mode == ExecutionMode::Hardened {
        for endpoint in [spec.start, spec.stop] {
            if let Some(value) = endpoint
    ...
    let mut stop = spec.stop.unwrap_or(len_i);
    if start < 0 {
        start += len_i;
    }
    if stop < 0 && spec.stop.is_some() {
...
fn basic_getitem(
    ...
    match array.shape.rank() {
        ...
        2 => {
            ...
            if array.order == MemoryOrder::C && slices[0].step == 1 && slices[1].step == 1 {
                let (row_start, row_end) =
                    basic_unit_slice_bounds(&slices[0], array.shape.dims[0], mode)?;
                ...
                for row in row_start..row_end {
                    let start = row * ncols + col_start;
                    values.extend_from_slice(&array.values[start..start + col_count]);
                }
                return Ok(CoreArray {
                    shape: Shape::new(vec![row_count, col_count]),
                    dtype: array.dtype,
                    values,
                    order: MemoryOrder::C,
                });
            }
            if array.order == MemoryOrder::F && slices[0].step == 1 && slices[1].step == 1 {
                let (row_start, row_end) =
                    basic_unit_slice_bounds(&slices[0], array.shape.dims[0], mode)?;
                let (col_start, col_end) =
                    basic_unit_slice_bounds(&slices[1], array.shape.dims[1], mode)?;
                if col_end - col_start == 1 {
                    let row_count = row_end - row_start;
                    let start = col_start * array.shape.dims[0] + row_start;
                    return Ok(CoreArray {
                        shape: Shape::new(vec![row_count, 1]),
                        dtype: array.dtype,
                        values: array.values[start..start + row_count].to_vec(),
                        order: MemoryOrder::C,
                    });
                }
            }
            if array.order != MemoryOrder::F && slices[1].step == 1 {
                ...
                let col_count = col_end - col_start;
                let ncols = array.shape.dims[1];
                let mut values = Vec::with_capacity(rows.len().saturating_mul(col_count));
                for row in &rows {
                    let start = row * ncols + col_start;
                    values.extend_from_slice(&array.values[start..start + col_count]);
                }
                return Ok(CoreArray {
                    shape: Shape::new(vec![rows.len(), col_count]),
                    dtype: array.dtype,
                    values,
                    order: MemoryOrder::C,
                });
...
mod tests {
    ...
    fn basic_getitem_scalar_reference(
        array: &CoreArray,
        slices: &[SliceSpec],
        mode: ExecutionMode,
    ) -> ArrayApiResult<CoreArray> {
    ...
    #[test]
    fn basic_getitem_c_unit_slices_match_scalar_reference_bits() {
        let values = [
        ...
        for slices in cases {
            ...
            let actual =
                basic_getitem(&array, &slices, ExecutionMode::Strict).expect("unit-step slice");
    ...
    #[test]
    fn basic_getitem_f_single_column_unit_rows_match_scalar_reference_bits() {
        let array = CoreArray {
            shape: Shape::new(vec![4, 3]),
            dtype: DType::Float64,
            values: vec![
                ScalarValue::F64(f64::from_bits(0x7ff8_0000_0000_0042)),
                ScalarValue::F64(-0.0),
        ...
        for slices in cases {
            ...
            let actual = basic_getitem(&array, &slices, ExecutionMode::Strict)
                .expect("F-order single-column slice");
            assert_eq!(actual.shape, expected.shape);
            assert_eq!(actual.dtype, expected.dtype);
        ...
        .expect_err("column bounds must remain checked after an empty row slice");
        assert_eq!(hardened_error.kind, ArrayApiErrorKind::InvalidIndex);
    ...
    fn basic_getitem_row_major_strided_rows_unit_columns_match_scalar_reference_bits() {
        let values = [
            ...
            ScalarValue::F64(10.0),
            ScalarValue::F64(11.0),
            ScalarValue::F64(12.0),
            ScalarValue::F64(13.0),
            ScalarValue::F64(14.0),
            ScalarValue::F64(15.0),
            ScalarValue::F64(16.0),
            ScalarValue::F64(17.0),
