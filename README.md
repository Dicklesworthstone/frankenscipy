...
| `eigsh` (symmetric sparse) | Thick-restart Lanczos (Wu & Simon) with full reorthogonalization over SciPy's `ncv = min(n, max(2k+1, 20))`, and ARPACK's relative convergence test; `converged` is the kernel's test AND an explicit residual check. It replaced a single-pass Krylov window, and before that the deflated power iteration whose constant-seed bug on path Laplacians is described under **Case Studies**. |
...
| `lgmres` | Augmented Krylov subspace built from prior approximation-error vectors, which enter the Arnoldi basis as in SciPy; convergence is judged on the true residual with SciPy's `‖r‖ ≤ rtol·‖b‖`. |
...
| RK23 / RK45 | Bogacki–Shampine 3(2) and Dormand–Prince 5(4) with SciPy's `select_initial_step`; `t_eval` samples and events use SciPy's cubic / quartic dense output. `nfev` counts as SciPy counts it. |
| DOP853 | Dormand–Prince 8(5,3), 12 stages, with SciPy's blended E5/E3 error norm; the derivative at the new point is evaluated explicitly (the method is not FSAL). The 7th-order dense output is SciPy's, including its three extra stages, which are evaluated and counted only on steps that are sampled. |
| BDF | SciPy's variable-order (1–5) NDF step for step: `num_jac` finite-difference Jacobian taken at `t0` and refreshed at most once per step, the LU kept across error rejections, SciPy's dense output from the difference array. On the live diff (`diff_integrate_solve_ivp_stiff.rs`) it reproduces SciPy's `nfev`, `njev`, `nlu` and step count. A user Jacobian cannot be supplied yet. |
| Radau | SciPy's 3-stage Radau IIA step for step (`crates/fsci-integrate/src/radau.rs`): Newton in SciPy's transformed variables with a real and a complex factor, the start extrapolated from the previous step, the predictive step-size controller, SciPy's dense output. It reproduces SciPy's counts on 7 of the 8 live-diff rows; the eighth spends one more Newton iteration. |
...
| `quad` | QUADPACK ported routine by routine (`crates/fsci-integrate/src/quadpack.rs`): `qagse` (GK21 with Wynn-epsilon extrapolation), `qagie` for infinite ranges, `qagpe` for break points, and the weighted `qawoe` / `qawfe` / `qawse` / `qawce`; `limit` is the number of subintervals, as in SciPy. |
...
| G3 | `live-SciPy diff (<shard>)` | The 753 `diff_*` files in 15 shards, each binary built and run with `FSCI_REQUIRE_SCIPY_ORACLE=1` against the pinned SciPy |
...
| G9 | `tolerance-policy ratchet` | `tolerance_lint --max-violations 360` over the `FSCI-P2C-*.json` fixtures (threshold may only fall) plus the harness ratchet: every `tests/diff_*.rs` tolerance contract (`const *_TOL: f64` and bare `<= <literal>` comparisons) is checked against the committed `fixtures/tolerance_baseline.json` — a loosened value, an unanchored contract, or a stale baseline entry fails the build, and `--write-baseline` refuses to adopt loosened values (frankenscipy-6a5s9). A tolerance passed as a literal argument to a comparison helper is not a contract the ratchet can see; the harnesses that still do this are listed in `frankenscipy-el4n5` |
...
The first fully green full-fan-out CI run completed on 2026-09-08: workflow run [`34180840286`](https://github.com/Dicklesworthstone/frankenscipy/actions/runs/34180840286) passed all 43 jobs across G1–G9 with zero failures. That is not a standing guarantee: of the 16 scheduled fan-outs after it, 7 were green and the runs of 2026-09-22 and 2026-09-23 were red. The next fully green fan-out was the scheduled run [`36123052714`](https://github.com/Dicklesworthstone/frankenscipy/actions/runs/36123052714) on 2026-09-25: 45 of 45 jobs, including the no-SciPy control.
