# PRE-REGISTERED: `dblquad_many` versus vectorized live SciPy
...
`docs/perf_ledger_cc.md` reports `dblquad_many` as 62.7--211x faster than a
Python loop over `scipy.integrate.dblquad` for a parameter sweep of smooth
Gaussian integrals. The historical comparison exposed every adaptive node of
every parameter value to a scalar Python callback and then ran the parameter
values serially. It was not a same-invocation paired experiment and did not
screen SciPy's public vector-valued integration APIs. It also lacked independent
A/A controls, executable identity, observed task count, and the corrected
median-CI gate. The old values are routing evidence, not a current incumbent
claim.
...
inlines the integrand, whereas the historical SciPy arm paid nested Python
callback cost for every parameter value. SciPy 1.17.1 now exposes public
`quad_vec` and `cubature` routes that can evaluate all parameter values in one
NumPy callback. That removes the exact per-parameter callback-tax mechanism.
This experiment therefore predicts a collapse rather than assuming the old win
will survive.

## Whole job and public incumbent screen

The named job is a 128-member two-dimensional parameter sweep:

`I(p) = integral_0^1 integral_0^1 exp(-p*((x-0.5)^2 + (y-0.5)^2)) dy dx`

for 128 deterministic `p` values spanning `[5, 40]`. Construction of the
parameter vector is outside timing. A timed arm must compute and materialize all
128 integral values, all available error/convergence records, a stable checksum,
and the `(p, I(p))` pair with the maximum integral. Thus the benchmark is the
whole parameter-study job, not one quadrature kernel.

Both implementations use the public absolute and relative tolerances
`1.49e-8`, unit-square bounds, and no cached result. Before effect timing, the
harness screens these semantically valid public SciPy 1.17.1 arms on the exact
fixed job:

1. a scalar Python loop over `scipy.integrate.dblquad`;
2. nested `scipy.integrate.quad_vec` calls whose NumPy callback returns all 128
   outputs together;
...
FrankenSciPy uses `dblquad_many` on the identical function and parameter vector.

The pre-timing scientific gate fails closed unless:

- both arms materialize exactly 128 finite integral values;
- FrankenSciPy reports 128/128 converged results and the selected SciPy result
  reports success;
...
- both arms have maximum scaled error at most four against the closed-form
  reference
  `pi/p * erf(sqrt(p)/2)^2`, evaluated outside timing; and
- the selected maximum occurs at the same parameter and its two integral values
  differ by no more than the sum of their requested absolute/relative
  tolerances.
...
**P1 -- strongest SciPy arm.** Predict a single-worker vectorized `cubature`
arm (`gk21` or `genz-malik`) wins the public SciPy screen. Falsified if scalar
`dblquad`, nested `quad_vec`, or the workers-map arm is fastest.

**P2 -- callback-tax headline collapses.** Predict the paired
`SciPy / FrankenSciPy` 95% CI has an upper endpoint below `21.1x`, more than a
tenfold collapse from the historical 211x headline after the incumbent removes
...
nested-scalar-per-parameter implementation. This is a deliberately directional
prediction made before measurement. If FrankenSciPy wins, retract it plainly.

**P4 -- removed callback tax is visible.** Predict the scalar-loop
`dblquad / selected-SciPy` screen-median ratio exceeds `10x`. This directly
...
independent FrankenSciPy/FrankenSciPy and 15 independent SciPy/SciPy A/A
ratios. The effect is the median of per-round `SciPy / FrankenSciPy` ratios. A
...
Choose FrankenSciPy `dblquad_many` for this exact 128-parameter job only if its
...
the old 62.7--211x public magnitude for this workload rather than falling back
